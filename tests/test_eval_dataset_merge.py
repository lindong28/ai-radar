...
def test_merge_duplicate_versions_and_reextend_without_original_archives(tmp_path):
    ...
    first = leaves(ob.build(admission_benchmark="aihot-prefilter", **args, version="v1"))
    second = leaves(ob.build(admission_benchmark="aihot-prefilter", **args, version="v2"))
    ...
    merged = leaves(ob.build(admission_benchmark="aihot-prefilter", bases=[first["visible-score"], second["content-enrichment"]],
    ...
    again = leaves(ob.build(admission_benchmark="aihot-prefilter", bases=[merged["news-admission"], first["news-admission"]],
...
def test_extend_with_new_raw_adds_questions_and_new_variant_removes_only_pair(tmp_path):
    args = setup_build(tmp_path)
    base = leaves(ob.build(admission_benchmark="aihot-prefilter", **args, version="v1"))["news-admission"]
    write_raw(args["raw_root"], "2026-09-18T08:00:00Z", "2026-09-18T08:15:00Z", [raw("b")])
    extra = leaves(ob.build(admission_benchmark="aihot-prefilter", **{**args, "start": "2026-09-18T08:00:00Z", "end": "2026-09-18T09:00:00Z"},
    ...
    changed = leaves(ob.build(admission_benchmark="aihot-prefilter", **{**args, "start": "2026-09-19T08:00:00Z", "end": "2026-09-19T09:00:00Z"},
...
def test_legacy_base_uses_unfiltered_observations_not_candidate_rows(tmp_path):
    ...
    latest = leaves(ob.build(admission_benchmark="aihot-prefilter", **args, version="v1"))
    result = leaves(ob.build(admission_benchmark="aihot-prefilter", bases=[Path(old["datasets"]["visible-score"]), latest["news-admission"]],
    ...
    old_only = leaves(ob.build(admission_benchmark="aihot-prefilter", bases=[Path(old["datasets"]["visible-score"])], version="v3",
...
def test_current_source_contract_can_remove_all_old_questions(tmp_path):
    ...
    result = leaves(ob.build(admission_benchmark="aihot-prefilter", bases=[first["news-admission"]], version="v2",
        data_root=args["data_root"], contract_path=args["contract_path"]))
...
def test_cli_base_only_and_invalid_arguments_and_corrupt_base(tmp_path):
    ...
    with pytest.raises(ValueError, match="together"):
        ob.build(admission_benchmark="aihot-prefilter", bases=[base], raw_root=args["raw_root"], version="v3")
    (base / "evidence/raw-inputs.jsonl").write_text("{}\n")
    with pytest.raises(ValueError, match="integrity"):
        ob.build(admission_benchmark="aihot-prefilter", bases=[base], version="v3", data_root=args["data_root"])
...
def test_new_reference_conflict_rechecks_old_field_without_dropping_others(tmp_path):
    ...
    result = leaves(ob.build(admission_benchmark="aihot-prefilter", bases=[base], references=[ref], version="v2", data_root=args["data_root"],
