...
def test_interpret_runner_never_trusts_the_cache_for_a_long_form_wechat_url(
    ...
    db_path = _seed_runner_db(tmp_path)
    assistant_root = _assistant_root(tmp_path)
    long_url = "https://mp.weixin.qq.com/s?__biz=MTQzMjE1NjQwMQ==&mid=2656194904&idx=4&sn=abc"
    with _connect(db_path) as seed:
    ...
    def fake_run(cmd: list[str], **kwargs: Any) -> subprocess.CompletedProcess[str]:
        ...
        if "--check-url" in cmd:
            ...
            return subprocess.CompletedProcess(
                ...
                0,
                stdout=json.dumps(
                    {"found": True, "title": title, "slug": "an-older-repost", "summary_file_path": str(other_summary)},
                    ensure_ascii=False,
                ),
                stderr="",
            )
