...
目标原文、数据清点、达标线、五条轴的处置、量具纪律、外层归因回路都在那里，此处只放不可省的七条：
...
- **选题范围与编辑口味上，我方定位与 AIHOT 冲突时一律取能对齐的那条**（用户 2026-09-11 常设指示，
  ...
  **而且判得对**——是我方口径窄，不是判错），用户裁定放宽。
  ⚠️ **它没有裁定 `方法忠实 vs 结果逼近` 那个分岔**（2026-09-11 由 review-gate 报出，本条第一版把它读进来了）。
  照抄参照物的决策函数、与调机制使读数贴近，**两条都是"对齐 AIHOT"**，差的是**怎么**对齐；
  而用户那句话讲的是"要不要对齐"。那个分岔**仍然悬着，要用时先问用户**。
  此前已定的两条边界不受本条影响：**「按目标比例硬配额」仍被禁**（硬编码统计量、池子一变即失效），
  允许的是**改判据本身**——以输入为自变量的规则（区别见
  `~/.claude/references/prompt-distribution-fitting.md`）。
