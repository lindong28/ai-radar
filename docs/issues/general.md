...
- **订正 2026-09-06**：本条最初写的第一条闭合方向是"在 prompt 里禁止输出品牌名作为标签"。**那是错的，方向相反**——查过 AIHOT 自己的标签后：它 32 个标签里有 8 个就是公司名（OpenAI 101 次、Anthropic 67、xAI 50、Meta 22、Google 17、Microsoft 10、Hugging Face 9、DeepSeek 7），占全部标签实例的 19%。禁止品牌名会让我们偏离拟合目标。
- **AIHOT 的真实规则（逐条比对 12 个被拒条目得出）**：品牌标签**只用一份约 10 个 AI 实验室的固定名单**，名单外的品牌一律改用主题标签——「小米人形机器人」它打 `产品更新/具身智能` 而不是 `小米`；「英伟达黄仁勋之女来华」打 `具身智能/行业动态`；而「微软升级 Win11」它**确实**打了 `Microsoft`，因为 Microsoft 在名单里。**我方 35 项受控词表与 AIHOT 的 32 项共有 30 项**，那份名单本来就已经对齐了。
- **所以问题是模型没照给它的词表办**，不是词表错了。两个标签相关的失败桶合计 172/204（84%）：117 条输出了名单外品牌，55 条输出的标签数不在 2–4 之间（`normalize()` 第 106 行）。
- **闭合方向（按对拟合目标的贡献排，均未实施）**：见下方「可选方案」小节。

#### 处置（2026-09-06，用户裁定：一切以拟合 AIHOT 为最高优先级）

**已实施 A + D（全对齐）。** 用户明确「保留我站特色不是现阶段目标」，故词表按 AIHOT 全对齐：加 `语音`/`政策/监管`，删 `DeepMind`/`GitHub`/`RAG`/`arXiv`/`搜索`。**改后两侧词表差集为空**（我方 32 项 = AIHOT 观察到的 32 项）。

实施要点两条，都不是照着方案描述直接能写对的：

- `deterministic_tags` 的守卫用的是 **v1** 词表（仍含 `GitHub`/`arXiv`），只改 V2 常量删不掉它们——我方输出里的 13 次 `arXiv`、7 次 `GitHub` 正是它按 host 加的。故在 v2 的 `normalize()` 里对确定性标签也过一遍别名映射与 V2 词表过滤。
- `arXiv` 映射到 `论文/研究` 而不是丢弃：AIHOT 用后者 33 次、从不用前者，映射比丢弃更贴近。

**实测效果**（`eval-fit run --limit 300 --seed 7`，与基线同子集可比）：
...
10 个拟合指标 8 升 2 微降，全部落在 n=300 的噪声内（无一判为 improved 或 regressed），**8 条达标线全部通过**。结构性恢复是确定的；拟合增益方向为正但在这个样本量下不可检出。

**剩余 8 条失败**：7 条标签数不在 2–4（**方案 B 才治**）、1 条 `why_recommend` 超 90 字符。

**待评估**：方案 B（prompt 里写明 AIHOT 的品牌规则）的剩余空间。现在的证据是——越界类已归零，B 在标签维度上能再拿的主要是那 7 条计数违规，以及让模型**主动**产出 AIHOT 式标签（而非事后修补）带来的 Jaccard 提升，后者未测。

**删掉的 5 个词，若日后想加回来**：`arXiv`（13 次）与 `GitHub`（7 次）对技术类内容真有区分度，是最可能值得恢复的两个；`DeepMind`(1) / `RAG`(0) / `搜索`(0) 实测几乎不产生。恢复的前提是「拟合 AIHOT」不再是最高优先级，或 AIHOT 自己开始使用它们。

#### 实施后的 review findings（2026-09-06 中档 review gate）

**已修 5 条**（各配一条经突变验证有区分力的测试）：

- **HIGH · 词表同时是读取期校验器**。`EnrichOutputV2.tags_must_be_known_and_unique` 也用这份词表校验**从库里读出来的行**，而 `parse_enrichment` 吞掉异常返回 `None`。实测 `data/radar.db` 与生产发布件各有 **279 行**带已删词（GitHub 112 / arXiv 93 / DeepMind 36 / 搜索 26 / RAG 14），改动后它们的整条 enrichment——中文标题、摘要、推荐理由、分类、全部标签——会在页面上消失，回落成英文原标题。且**永不自愈**：`current_version_v2()` 是常量，runner 用 `NOT EXISTS(... error IS NULL)` 排除已成功行。**这正是方案 A 要消灭的失败形态，被 D 从写入期搬到了读取期，还作用在已付过费的存量数据上。** 修法：拆开读写两个词表——`_READABLE_VOCABULARY_SET = 写入词表 ∪ 已删词`，读取接受写入曾接受过的一切，只有写入收紧。验证：279 行解析失败数 279 → **0**，而写入侧 `['arXiv','GitHub','智能体','推理']` 仍产出 `['论文/研究','智能体','推理']`。
- **MEDIUM · 确定性层可单独满足下限**。丢弃后的「≥2 个标签」检查算的是模型标签 + 确定性标签的合集，而后者完全不依赖模型输出。实测 5000 条已 enrich 的 item 里 **1.6%** 的确定性层单独就能产出 2 个词表内标签——于是「模型一个合规标签都没给」与「模型给对了两个」在这条闸上读数相同，摘要与理由照样保留。改为要求模型自己至少落一个词表内标签。
- **MEDIUM · 身份锚只覆盖两个渲染常量中的一个**。`prompts_v2.py` 同时渲染词表（normalizer）与 `PRIMARY_CATEGORIES`（`classification.py`），新加的摘要只哈希前者；且 normalizer 模块本身不在任何身份字段里——**方案 A 若单独提交，`stage_identity` 每个字段都会逐字节不变**。改为同时哈希两个渲染常量与两个行为模块文件。
- **MEDIUM · `stage_identity_diff` 无人读，且对旧基线产生噪声**。它只写进 `metrics.json`，`report.md` 与 CLI 都不显示；且旧基线缺少新字段时三个 stage 全被标为"变了"，真正变了的那个与纯键缺失的两个不可区分。改为只比对两侧都有的字段，并在报告与 CLI 显式提示。
- **LOW · 渲染期与写入期对同一确定性标签给出不同结果**。`topic_tags_v2`（渲染期）只做词表过滤、不做别名映射，于是 arxiv 源的条目在渲染期丢弃 `arXiv`，而写入期把它映射成 `论文/研究`。改为渲染期对确定性标签同样走别名映射；**存量标签则按 readable 词表渲染**——与 HIGH 那条同一条原则。

**记账不修**（按最低充分方案；每条带失败场景）：

- **标签数不在 2–4 仍整条拒收**（55/204，A+D 之后剩余 8 条里的 7 条）。reviewer 指出 A 的理由逐字适用于它，且 `merged[:4]` 已证明「截断」是可接受处置。**未纳入本轮是因为用户裁定先做 A、再量 B 的剩余空间**，这一条正落在那个待测区间里。失败场景：模型返回 5 个标签、其中 4 个合规 → 仍整条拒收。
- **丢弃动作无遥测**。`dropped` 只在拒收路径进异常字符串；成功路径不计数、不落字段。**产生这次改动的那条测量通道被这次改动关闭了**——117 这个数拿得到，正因为原来会拒收并把越界标签写进 `error`。失败场景：上游模型轮换后 50% 标签越界 → 页面标签质量下降、Jaccard 下滑，而 `failures.enrich.errors` 为 0、无日志、无告警。
- **词表比对前无大小写归一**。实测越界样本含 `NVIDIA` 37 / `Nvidia` 16。改动前触发重试，改动后静默丢弃。
- **A 的实际回收条数无法回放核实**：被拒行只存 `{"attempts": 2}`，模型原始响应已丢弃。可核的只有「110/117 只越界一个标签」。且接收行的原始标签数分布是 `{2:792, 3:952, 4:793}`，约 30% 只给 2 个标签——这部分丢掉 1 个后剩 1 个，现在会被新增的「模型至少贡献一个」检查拦下。**实际回收率低于 117**，具体数未测。
- **前端分区标签表仍列 `搜索`、`arXiv`**（`web/static/app.js`），改后不可能再产生。无功能破坏（`产品更新`/`论文/研究` 仍匹配），属陈旧配置。
- **`canonical_tag_map_bytes()` / `TAG_MAP_SHA256` 是死代码**（全仓仅定义处命中），且与新增的身份摘要构成同一事实的两个权威。
- **`CONTROLLED_VOCABULARY_V2` 的推导式无三项保护**：additions ∩ removals 非空时 removal 静默胜出、无去重、无「v1 新增词会静默进 V2 与 prompt」的断言。今天三者都为空/无冲突，但无断言钉住。

#### 可选方案与取舍（目标：在相同输入上尽可能拟合 AIHOT 的输出）

| 方案 | 对拟合的作用 | 确定性 | 代价 / 风险 |
|---|---|---|---|
| **A. normalizer 丢弃名单外标签、不整条拒收** | **高**。110/117 只越界 1 个标签，丢掉它剩下的正是主题标签——而那恰好是 AIHOT 打的。同时把这 117 条的摘要/理由/分类一并救回 | **确定**，不依赖模型行为改变 | 改生产 normalizer；掩盖模型不合规（但从拟合角度不重要——要的是输出像 AIHOT，不是模型守规矩）。不解决 55 条标签数违规 |
| **B. prompt 里写明 AIHOT 的品牌规则**（名单外品牌改用主题标签） | **理论最高**——直接复制 AIHOT 的决策函数，而不只是事后修补 | **不确定**，要跑了才知道模型照不照做 | `prompts_v2` 是**一个** prompt 产出全部字段，改它会同时动摘要/理由/分类，可能在别的维度回归 |
| **C. 扩大词表，把 NVIDIA/小米/Amazon 加进去** | **负的——唯一会让拟合变差的方案** | 确定变差 | 加了之后模型会"合法地"输出 `NVIDIA`，而参考是 `具身智能/行业动态`，Jaccard 直接掉。这是最直觉的方案 |
| **D. 两侧词表对齐**：加 `语音`/`政策/监管`，去 `DeepMind`/`GitHub`/`RAG`/`arXiv`/`搜索` | **小而确定**。我方 5 项 AIHOT 从不用（实测 21 次实例，占我方标签 1.3%，每次必然拉低 Jaccard）；AIHOT 2 项我方没有（21 次实例，算指标时被丢弃） | 确定 | 删掉的 5 项对**产品**可能仍有价值（`arXiv` 13 次、`GitHub` 7 次），这是「拟合 AIHOT」与「我站自己的信息组织」之间的真实取舍 |
