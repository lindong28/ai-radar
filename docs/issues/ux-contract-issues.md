...
### 精选排序的契约描述落后于实现两处（2026-09-10，候选，未直改契约）

`ux-contract.md:158` 与 `:434` 都写「5 维评分（相关性、信息密度、时效性、权威性、工程价值）」，而：

1. **第 6 维 `significance` 是 2026-09-06 加的**（ADR-20260906-7c31），且它是当前权重最大的一维（0.50）。这条漂移**早于本次改动**，此前无人记。
2. **2026-09-10 起排序另有一项类别系数**（`select.CATEGORY_MULTIPLIERS`，论文 0.95），它不是一个评分维度、只作用于排名先后，见 ADR-20260910-3f8b。`web/templates/about.html` 已向读者解释它（含"会影响显示分与徽章、不影响 6.5 阈值"）。

两处都只改动了**面向读者的解释文案**，未改契约里的可观察行为条款（精选每天 30–40 条、阈值 6.5、搜索排序规则等均未动）。按 docs-organization-protocol §4.6，自由 session 不直改 ux-contract，故记为候选：下次走 ux-contract 工作流时把「5 维」改为「6 维」并决定要不要把类别系数写进契约。
