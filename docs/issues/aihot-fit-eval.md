...
**第二重要的一条**：`selected` 只有 3/8 自洽。这独立解释了 ADR-499e 记的那个失败——按 `selected` 拟合的权重换 seed 就崩，此前归因于"78 个正例撑不起搜索"，现在看**目标本身就不稳定**，样本量只是其中一半原因。这条也意味着 `selected_auc = 0.7907` 可能已经接近它能到的位置。
...
**仍然没有读数的**：`tag_jaccard`（t5 那批不带 tags 字段，8 对里无可比样本）与 `reason_closeness`（同样只有 1 对且一侧为空）。这两维的天花板仍然未知。
...
**外部性的实测覆盖只有 16 分钟，其余靠推断。** 我另起的只读记录器在 15:31–15:47 采到 9 个样本，最长观测到停滞 **9,523 秒（2 小时 39 分）**；此后到 19:29 的 111 个样本**全部记 0**——因为我在 15:47 重写了探针 CLI（`score` 由位置参数改为 `--stage`），而记录器仍在传位置参数，`argparse` 报错、输出被 `sed` 吞成空、落盘为 0。**"接口变了之后仪器继续报正常"，与"确实没有饥饿"完全同形**，这是本轮第四次同形态失败，且这次是我自己改的接口打瞎了自己的记录器。所以 ISSUE-FIT-29 的事故时长以用户的消费者侧读数与 pipeline 日志为准，记录器只补了开头那一段。
