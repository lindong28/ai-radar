...
### docs/CLAUDE.md 的 ADR 清单漂移了三条（2026-09-10 发现，未修）

`docs/CLAUDE.md` 顶部写着「docs/ 下新增、重命名或删除文档时同步更新本索引」，但它的 ADR 表缺
`20260906-7c31`、`20260907-a1c4`、`20260910-3f8b` 三条——都是近几天新建的。本轮新建的 `20260910-9e21`
已按规则加进去了，那三条**不是本轮的改动，未代改**（按 CLAUDE.md「只修改能追溯到用户需求的内容」）。

**为什么值得记**：`docs/adr/README.md` 自陈是「单一权威：每条决策的标题与状态只在该索引维护」，而
`docs/CLAUDE.md` 又独立维护一份文件清单。两份清单对同一批文件，其中一份不含状态、只含文件名与一句说明——
于是它漂移时没有任何东西会失败，而读 `docs/CLAUDE.md` 找 ADR 的人会得到一份不完整的清单。
要么让它只指向 `adr/README.md` 而不再逐条列，要么给它一个一致性检查。
