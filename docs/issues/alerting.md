...
- **审查另外指出、本次未处置的**：恢复消息与事故账本会把 episode 时长**多算**一到两次评估的滞回，而系统知道何时转安静却不写进消息（P8/§1）；滞回期间管理面 `detail` 仍显示旧的故障文案、而 `last_evaluated_at` 是新的（两种状态同一呈现）；滞回只覆盖七条 ok 路径中的一条，其余（A7 全暂停关闭、severity 交接）不经过它——今天无影响，是下一个开启该选项的规则的陷阱。
