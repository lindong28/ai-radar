...
## ISSUE-ALERT-20260911-7c41 · A5 持续 firing 却未送达，`/wechat` 停更 5 天靠用户肉眼发现

**状态**：open · **优先级**：high

`/wechat` 自 2026-09-06 停更（interpret 因收据闸失效而每轮静默跳过）。A5 **判定正确、也在持续 firing**——`logs/alert-check.log` 里有 `A5 firing 微信解读产出停滞 - 已 100.0 小时无成功解读；等待文章 83 篇`——但同一批日志行显示投递计数从 `sent=3` 落到 `sent=0`，此后用户再未收到任何提示，直到 5 天后自己发现页面不动。

这与 ISSUE-A01 的形态不同：那条是**判定**被假 ok 打断，这条是判定一路正确、**投递**被去重压住。故障持续越久，去重越沉默，正好与需要的方向相反——一个已经响了 100 小时的告警比刚响的那次更需要被看见。

**未在本轮闭合**：本次 session 的目标是恢复 `/wechat`（已恢复，收据闸按所有者裁定移除，见 CHANGELOG 2026-09-11），投递去重不在其范围内，独立于该次改动而成立。

**闭合方向**：长时持续 firing 需要有 re-notify 阶梯（如按 4h / 24h / 72h 递增重发），而不是首次投递后永久静默；参考 `~/.claude/references/service-operations-protocol.md` §6 的去重契约。相关：memory `airadar-wechat2rss-replaces-mp2rss` 记过 healthcheck dedup 曾永久静默，是同一类。
