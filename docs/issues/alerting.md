...
- **A2 恢复滞回已实现（2026-09-09），但用户授权时那个理由是假的，据实订正**。第一版是墙钟迟滞，独立审查判否，三条我都自己复核过：
  - **理由不成立**：我把上面 C1 那个「盲窗 → 假 resolve」当成了实现它的依据，但那是**带出网开火支路的版本**才有的形态，而那个版本已经撤了。实测当前 A2：出网故障下 preflight 可读 / 盲窗(`status=''`) / 读到 healthy **三种情况全部 `firing=True`**——**盲窗根本翻不动 A2**，它的开火位来自 `minutes_since_successful_pipeline > no_success_minutes`，preflight 只决定归因**文案**。
  - **真正成立的理由在别处**（审查独立复现，且仓里早有记载）：`thresholds.py` 的 `stage_p95_latency_ms` 旁边写着，后台阶段 P95 在 2h 窗口里只有 ~10–20 个样本、被 1–2 次尾延迟主导，**反复穿越阈值造成 firing/resolved 交替**。加滞回后同一次抖动 🔴×5/✅×4 → 🔴×5/✅×0，`since` 保持在真实起点。
  - **机制也换了**：墙钟迟滞在**评估器停摆**时会自己走完，于是停摆后的第一次读数就宣告恢复——而评估器停摆与它要报的故障正相关（P9 的反面）。改为**数连续不 firing 的评估次数**（`a2.resolve_debounce_rounds`=2，默认 0 即其余规则不变），计数在无人观测时不前进。这是照仓内既有的 A4「无状态滞回」（`account_resolve_rounds`）抄的形状。
  - **测试也是空的**：审查实测，删掉时长比较后我那三个测试**全过**——它们只断言了「至少两次安静」，从没测过"持续时长"本身。改成计数后这个性质可测了，那三个当初存活的变异（删比较、阈值降为 1、边界 `<`→`<=`）现在**各自都让测试变红**。
- **同一天第二次「自评通过、独立审查判否」**（前一次是本条的提前触发）。两次自评都是「测试全绿 + 我自己挑的变异全被抓」，而两次审查都在实质上判否。**记下来的是这个形态本身**：作者做的变异测试只会变异自己想到过的东西，它测的是仪器灵敏度、不是覆盖面；在告警这个面上它已经连续两次给出假绿。
- **审查另外指出的三条，2026-09-09 逐条定归属**（此前只写了"未处置"，那不是归属）：

  | # | 问题 | 归属 | 依据 |
  |---|---|---|---|
  | ① | 恢复消息与事故账本把 episode 时长**多算**一到两次评估的滞回，系统知道何时转安静却不写进消息（P8/§1） | **记账后续做，需一次独立审查** | 见下方修法。不在本轮做的理由是可陈述的：改动要穿过 lifecycle 状态机与消息格式化两层共 5 处，而**作者在这同一个状态机上当日的自评准确率是 0/2**（两次自评通过、两次独立审查判否）。在这个面上作者自评不构成充分验证，硬做等于赌第三次。量级上也不紧：滞回 1–2 次评估 = 5–10 分钟，而事故时长以小时计（09-09 出网 9.9h）——**注意那次 9.9h 的误报与本条无关**，误差来自扫描时折叠了 SKIP 轮 |
...
## ISSUE-ALERT-20260911-7c41 · A5 按契约持续投递 100 小时，用户仍 5 天未察觉

**状态**：open · **优先级**：high · **注意：本条 2026-09-11 首版归因错误，已订正，别引用旧版结论**

### 已核实的事实

`/wechat` 自 2026-09-07 停更（interpret 因收据闸失效每轮静默跳过）。A5 判定正确、持续 firing，且**投递一直在正常发生**：应用层 `COOLDOWN = 30min`（`src/airadar/admin/alerts.py:42`），故障期间实测 `sent` 读数为 `2, 0, 0, 0, 0, 2`——每 15 分钟一轮、约每 36 分钟发一次，与 cooldown 节奏吻合。100 小时里大约投递了 200 次。

**用户仍然 5 天不知道，最后靠肉眼看页面发现。**

### 首版归因错在哪（留作教训）

首版写的是"投递被去重永久压住（`sent=3`→`sent=0`）"。那是把日志里 `{A1,A2,A3,A4,A5,A6,A7} sent=N` 这个**整轮所有规则的合计**当成了 A5 单独的投递计数，并且只取了两个相邻采样点就下结论。实际它在 0 与 2 之间交替，取任意单点都能"证明"想要的结论。

据此还差点改错代码：已在 `prepare_notification` 里加了一套 re-notify 阶梯（让 nonce 随 firing 时长递增），写完才发现应用层本就有 cooldown 重发、且 `test_cooldown_repage_allocates_new_notification_nonce` 早已覆盖该行为——那个改动修的是不存在的缺陷，已回滚未提交。

**判据教训**：`sent=N` 的**作用域**没有先核实（是整轮还是单规则），而两种读法在单个采样点上完全同形。

### 真正待查的问题（未验证，别当结论）

告警送达了却没有产生行动。候选成因，**均未验证**：

1. **告警疲劳**：同期 A7 常驻 firing（`xAI News 静默 67.4h`，消息里自带"无需处置"），符合 `alerting-review-principles.md` 原则 9 所说"常驻降级通知退化成噪声"的形态。若通道里长期有不需处置的 firing，A5 会被淹没。
2. **通道可达性**：投递目标通道用户是否实际在看，本轮未核实。
...
**2026-09-18 外部复盘回写（system-config session 测得，非本仓改动）**：本条「闭合方向」要的那次占空比测量已有读数，**修复生效，可见台阶落在 2026-09-14**。按 `data/alert-events.jsonl` 的逐日 firing 计数：A7 从 09-05..09-13 的 25–43 次/天降到 09-14 起的 0–5 次/天，且 severity 由 `page` 翻为 `notice`；A5 自 09-12 起为 0。投递侧独立佐证（`~/.local/state/im-notify/alert-sent.log`，按 `push=sent` 计）：09-07..09-13 共 510 条、73 条/天；09-14..09-18 共 297 条、59 条/天，其中 A7 仅 10 条、A5 为 0。

修复后通道里剩下的占空比**不再由本仓主导**：154/297（52%）是 system-config 的 `watchdog high-cpu`，62 条（21%）是 `ai-radar-raw-collection-Radar`，28 条（9%）是 `publish-demo` 两个 launchd 服务持续宕机，A2 25 条（8%）。high-cpu 已在 system-config 侧加跨轮去抖并降为 warning（未提交时点：2026-09-18）。

本条仍 open 的两项未变，且**本次都没有核实**：通道可达性（用户是否真在看那个 webhook）、A5 是否需要独立的持续时长升级路径。上面的读数只证明「不需处置的常驻 firing 已大幅减少」，不证明「用户现在看得见告警了」——那正是本条首版栽过的那种外推。

## ISSUE-ALERT-20260920-3e91 · A2 的「距上次成功」比日志实算高报 74–104 分钟，而阈值本身没问题

**状态**：open · **优先级**：high —— 它使 A2 在 pipeline 正常时每约 85 分钟 page 一次；本轮实测 12 小时内占全部飞书推送的 43%

### 已核实的事实

对 2026-09-20 的每一次 A2 firing，用 `logs/pipeline-*.log` 的文件名时间戳与 `=== PIPELINE DONE (failed=0` 行重建"那一刻真实的上次成功起始"：

| A2 firing | 它声称 | 日志实算 | 高报 |
|---|---|---|---|
| 02:49 | 138 分钟 | 34 | +104 |
| 04:21 | 125 | 36 | +89 |
| 05:48 | 122 | 48 | +74 |
...
02:49 那次可逐文件核对：`pipeline-20260920-021500.log` 有 `DONE failed=0`，其后 02:30 / 02:45 两轮 SKIP。按 `alerts.py` 的循环（newest-first，skip 计数后 continue，遇 `status=done and failed==0` 即 break）本应得 34 分钟。

**阈值不是原因**：`no_success_minutes` 默认 120，而成功轮 start 间隔中位数全历史 30 分钟、最近 12 小时 15 分钟，**最近 12 小时 0% 超过 120 分钟**（n=18）。120 是中位数的 4–8 倍。

### 排除项

- 不是日志目录不同：alert 服务 plist 的 `WorkingDirectory` 与 `cd` 都是仓根，`log_dir` 取默认 `PROJECT_ROOT/"logs"`，与上面重建所用的是同一批文件。
- 不是解析器：`PIPELINE_DONE_RE` 与 `SKIP_RE` 对真实行都匹配；`_load_pipeline_runs` glob 全部日志、按文件名排序，无截断。
- 不是时区：`started_at` 带 `+08:00`，与 `current` 同为 aware。
- **此刻复跑同一代码路径结果正确**：扫 2 个 run 即找到 12:15 的成功轮、`consecutive_skip_logs=0`、算出约 32 分钟。故本条**未能在当前状态下复现**。

### 待查（未验证，别当结论）

`consecutive_skip_logs` 在那几次都报 7，而 7 轮 × 约 15 分钟 ≈ 105 分钟，与高报量级吻合——提示它把日志里成功的那些轮当成 skip 跳过去了。但代码读起来不该如此，且现在不复现。

**闭合方向**：在 A2 的 detail（或一条 debug 记录）里带上它选中的那个 run 的 `started_at` 与文件名，下次 firing 即可与日志逐行对账，不必依赖事后重建。**不要在此之前改 `no_success_minutes`**——阈值已被上面的分布证明不是原因。

## ISSUE-ALERT-20260920-7d44 · 采集监督去抖遗留的四个边界（2026-09-20 review 裁决不修）

改动本身见同日 CHANGELOG。以下由独立 reviewer 报出、本轮不修，逐条带读数。

| # | 边界 | 读数 / 触发前提 | 候选修法 |
|---|---|---|---|
| F5 | streak 状态丢失或重新部署时，首次检查算出空失败集 → incident True→False → 推送一条「已恢复／本次健康检查通过」，而源仍然是坏的；约 15 分钟后再 fire | 本次部署侥幸躲过（`notification-Radar.json` 当时是 `incident:false`）。任何重部署、state 清理、或**手工删除损坏的 streak 文件**都会命中 | 恢复判定也需连续确认，而不是单次观测即清零；当前是"3 次才报、1 次即清" |
| F7 | A7 接不住单源抓取失败：其可评估集约占 40%（98 个源 unevaluable），2026-09-14 起单源降为 notice，阈值是 13–37 小时的内容静默而非分钟级抓取失败。`claude_youtube` 按 `docs/operations/continuous-eval-data.md` 是真坏（无条件 GET 仍 404、未找到替代），约一半轮次抓不到 | 去抖后若它不连续失败 3 轮就再也不报，而**没有任何规则度量抓取成功率** | 需要一个按源的成功率指标，而不是靠 A7 或本规则的连续性判据 |
| F8 | `--dedup-text str(incident)` 使新源加入已开事故时无声，而本改动把平均 episode 长度从 5.95 轮拉到 19.0 轮，暴露窗口长 **3.2×** | 机制既有（独立），放大由本改动引入 | dedup-text 纳入失败源集合的摘要 |
| F9 | 新加入或取消暂停的源会被旧 manifest 判为失败（`latest['sources'].get(slug)` 为 None）。改动前立即报且措辞准确；现在第 3 轮才报，且谎称"连续 3 轮以上" | 需要一次配置变更 | `expected` 与 manifest 取交集，或对 manifest 中不存在的 slug 单独措辞 |

### 追加（2026-09-20 修复闭环）：ISSUE-ALERT-20260920-7d44 的后续三条

第二轮 reviewer 在修复版上又报 6 条，全部由上一轮修复引入。按修复轮预算规则停止迭代：HIGH 那条（去重身份吞掉全盘失败）与两条「永久沉默」类已修并有变异读数，其余三条记账不修。

| # | 边界 | 读数 / 触发前提 | 候选修法 |
|---|---|---|---|
| N2 | 广度旁路是无状态的：27 份 all-160 manifest 里只有 **5 份**被 5 分钟采样网格看到，6 次广度事件中有 3 次只靠**单次**采样命中。cron 延迟、`read_run` 变慢或机器休眠都会让最该看见的事件变成沉默 | 当前 6 次全部上报，但余量只有一次采样 | 把「上一轮曾广度失败」记进 streak 文件，下一轮仍可说「上一轮 160/160 未收到」 |
| N3 | `write_streaks` 永不抛出，于是**持续**写失败（磁盘满、只读 FS、路径变成目录）会变成永久沉默：每轮读到陈旧文件、算出 stale+1、永远存不下去；无先前状态时所有源恒为 1，单源告警**永远不会 fire**，且无处上报 | 读失败是自愈的，写失败是自我延续的——两者不对称 | 保持不抛出，但追加一条 problem 行（如「Radar 去抖状态无法持久化」），既保住不抛的理由（避免取代整份 Radar 检查）又不至于黑掉 |
| N6a | pid 命名的临时文件在进程于 `open` 与 `replace` 之间被杀时残留，且每个新 pid 留一个，无上界（旧设计的固定临时名反而不会） | 需要在窄窗口被杀 | 启动时清理 `radar-source-streaks.json.*.tmp` |
| N6b | 广度消息会列出全部失败源，109 个源那次实测约 **1.9 KB**。此前那一轮是被静音的，所以从未发生；现在对最重要的事件是必然发生，且是这组消息里最难读的一条。另注：`notify_transition` 在 notifier 非零退出时抛 `RuntimeError`，而它在 main 的通知块里**不在任何 try 内**、且 Radar 排在 AIHOT 之前——若超长正文曾被拒，AIHOT 的通知也不会跑，整轮反复重试 | **未核实**飞书/im-notify 是否有正文长度上限；若有，本条升级为通知轮死锁 | 正文只列前 N 个源 + 总数；并核实长度上限 |

**已修、有变异读数支撑的**：去重身份纳入广度标志（裸布尔变异 1 failed）、`escalated` 恒 False（1 failed）、空 `run_id` 守卫（1 failed）。连同前一轮的阈值、损坏文件守卫、广度旁路、main 接线，**7/7 变异体击杀**，阴性对照 48 passed。

### 更正（2026-09-20 实测）：`claude_youtube` 不是坏源，上游端点在间歇抽风

`ISSUE-ALERT-20260920-7d44` 的 F7 行引 `docs/operations/continuous-eval-data.md`，称该源「无条件 GET 仍 404、未找到替代」，据此把它当作真坏。**实测不成立**：

对 `https://www.youtube.com/feeds/videos.xml?channel_id=UCV03SRZXJEz-hchIAogeJOg` 连续裸 GET 三次 —— `404 text/html` / `404 text/html` / **`200 text/xml`，24120 字节的真实 feed**。同一时刻对照一个已知有效的 YouTube 官方频道 feed（`UCBR8-60-B28hp2BmDPdntcQ`）返回 **500 text/html**。

即 channel_id 有效、内容取得到，失败来自 **YouTube feed 端点自身的间歇性故障**，与本源无关，也不是「找不到替代」能解决的。因此：
