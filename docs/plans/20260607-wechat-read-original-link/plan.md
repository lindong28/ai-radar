...
> 任务：让 https://aiplanet.live/wechat 的每篇文章解读页（及列表卡片）显式提供一个可点击的「访问原文」入口，跳转到对应微信公众号原文。
...
- **TODO 部署验证**：本地 V1–V7 过后，按既有部署流程（serve 重启 + tunnel；注意 pipeline cron 锁，见 docs/operations）上线，再在 aiplanet.live 复跑 V1–V3 抽样确认缓存已刷新。*Pre-set move*：若线上抽样仍取到旧 `?v=`（缓存未刷新），按 docs/operations 重启 serve 强制资源失效后复跑一次；仍不刷新才升级排查（如 tunnel/CDN 层缓存）。
