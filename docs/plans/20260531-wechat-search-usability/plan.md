...
用户在 your-domain.example 搜索框输入公众号名/作者想找该来源文章：
...
- **使用者 A（端用户）**：your-domain.example 访客，用搜索框按来源名/作者找文章。拿到结果用来**判断该来源近期发了什么、决定打开哪篇深读**。
...
> 视觉/主观兜底（可选人工 gate）：在 your-domain.example 或本地前端实际搜"十字路口/歸藏/归藏"，肉眼确认公众号文章出现在首屏。V2–V4 的 API 断言已先兜底主要风险，人工仅做最终体感确认。
