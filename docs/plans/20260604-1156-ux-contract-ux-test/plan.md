...
| 项 | 值 |
|---|---|
| 生产入口 | https://aiplanet.live |
| 分页页面 | `/all`（时间线）、`/wechat`（微信解读列表） |
| 本地 serve | `http://127.0.0.1:8000`（launchd KeepAlive，跑在本 checkout `/Users/lindong/research/ai-radar`） |
| 认证 | 公开只读，无需登录 |
...
访问 `https://aiplanet.live/all`，观察底部 `#pagination`（`.pagination`）。判据：
...
访问 `https://aiplanet.live/wechat`，观察底部分页。判据：
...
5. 越界 `https://aiplanet.live/wechat?page=9999` → 回到最后一页（第 4 页），**不**显示"暂无微信文章解读"空态。
