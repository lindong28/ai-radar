...
- **来源**：用户诉求「我希望 https://aiplanet.live/wechat 能支持搜索，有类似于精选和"全部 AI 动态"的搜索体验」。
...
- **使用者（端用户）**：aiplanet.live/wechat 访客，浏览"微信文章解读"精选库（当前 202 篇、10 个公众号）。
...
- **改了 Python 路由 → 必须重启 serve** 才生效。**重启踩坑（记忆 `wechat-interpretation-plan` 已记）**：生产 serve 启动会 `db.migrate()`；若 cron 跑 pipeline 持锁时重启 → `database is locked` → 站点 down。安全重启：先暂停 cron（注释 crontab `*/15 … pipeline.sh`）或等锁释放，再 `launchctl kickstart -k gui/$UID/live.aiplanet.ai-radar.serve`。
