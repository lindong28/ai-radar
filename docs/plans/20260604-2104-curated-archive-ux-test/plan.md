...
- 生产入口 `https://aiplanet.live`，精选页 `/`。本地 serve `http://127.0.0.1:8000`（launchd，跑在本 checkout，StaticFiles 即时生效）。
...
- **后端改 Python（curated 路由 / app.py）需重启 serve**（launchd KeepAlive，`launchctl kickstart -k gui/$UID/live.aiplanet.ai-radar.serve`）；注意 pipeline 可能持 DB 锁（fetch/prefilter），重启失败先查锁。
