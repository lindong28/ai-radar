...
`https://your-domain.example/` 当前是纯 CSR：HTML 是空壳，JS 解析后才发 `/api/v1/curated` 请求，浏览器经历"白屏 → 正在加载 → 内容"的两段等待，首屏可见耗时约 3 秒，spinner 中位数 1295ms。
...
- **使用者**：访问 your-domain.example 的最终用户（浏览器）。
...
| # | 维度 | 实施步骤 | 通过条件 |
|---|------|---------|---------|
| L2-1 | 首屏 spinner 消失 | Playwright 脚本打开 `https://your-domain.example/`，在 navigation start 后 200ms 内检测 `text=正在加载` selector，5 次测量。（注：脚本无法测"持续时长"，只测"是否出现"；本地必须 0 出现，生产允许 ≤ 1 次出现以容忍网络抖动。） | 本地 0/5 出现；生产 ≤ 1/5 出现。 |
...
| L2-7 | 视觉无回归 | Playwright 截图 `https://your-domain.example/`、`/all`、`/daily`、`/about` 各一张，与 `plans/20260528-ssr-preload/baseline-screenshots/` 下的改造前截图对比。**对比姿势**：两个浏览器窗口并排，宽度 ≥ 1280px，同一 viewport。按下表 4×4 评分。 | 16 格全部 PASS。任一格 FAIL 或 ≥ 2 格 borderline → 整体 FAIL。 |
...
| # | 步骤 | 内部 verify |
|---|------|------------|
| 0 | 截图 baseline：Playwright 打开生产 `https://your-domain.example/`、`/all`、`/daily`、`/about` 各截一张到 `plans/20260528-ssr-preload/baseline-screenshots/` | 4 个 PNG 存在 |
...
| 7 | 部署 + 生产验证：`launchctl kickstart -k gui/$UID/com.example.ai-radar.serve`，跑 §6 的 L2-1/L2-2/L2-4 Playwright 生产实测 | spinner ≤ 1/5 且 < 200ms；FCP 中位数 ≤ 1.5s |
...
URLS = [
    ('https://your-domain.example/', 1500),
    ('https://your-domain.example/all', 1500),
    ('https://your-domain.example/?category=ai-models', 1500),
    ('https://your-domain.example/?q=openai', 1500),
]
...
2. 左窗口打开 `plans/20260528-ssr-preload/baseline-screenshots/<page>.png`，右窗口打开生产 `https://your-domain.example/<page>`。
