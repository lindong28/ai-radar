...
│
├── curator/            # 阶段 4：精选
│   ├── select.py       #   curate 主流程：加权评分 + 排序期类别系数 + 新鲜度配额 + 来源形态配额 + 影子基线 + 去重 + 排名校准
...
│       └── cli.py      #     build / run / judge / report 分发
│
...
`pipeline.sh` 在外部阶段之前调用 `egress-preflight`：它经单一本地出口端口（`AI_RADAR_EGRESS_PROXY_PORT`，默认 59527）**实发一次请求**（目标 `AI_RADAR_EGRESS_PROBE_URL`，默认 `https://api.github.com/zen`，特意与被诊断对象无关），拿到任何 HTTP 响应才继续——拿到响应即证明 CONNECT 成功、隧道通得了 TLS。**不用 TCP 连接探针**：那区分不了「本地 listener 活着」与「上游隧道通」，2026-08-18 实测一次 listener 正常而同轮 162 源全 `Connection refused`（CHANGELOG 2026-08-18）。2026-09-09 前它改为严格解析 `check-proxy-status --format=kv` 的十二个字段；换掉是因为那读的是 selector 的存储模式而非路径，会在代理正常服务时 fail-closed（当天实测 9 小时），且那次假阴性与真故障不可区分。代价是应用不再对逐 hostname 的线路有 fail-closed 契约——routing 由监听该端口的东西决定。应用不读取 `AI_RADAR_PROXY_FILE`、`current-proxy`、假定导出的 selector 环境变量或父进程六个 proxy 变量。
