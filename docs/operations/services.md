...
评测原始数据的留档与冻结入口见 [持续采集说明](continuous-eval-data.md)。本机已启用 Radar 过滤前归档和 AIHOT 缺窗模式，并经批准安装每日 03:07 的 Radar 原始数据 30 天清理（冻结集排除，复用 run-or-alert）。是否形成完整评测窗口须另据实际归档审计，不能只凭 pipeline 持续运行判断。
