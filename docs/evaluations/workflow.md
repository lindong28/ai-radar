...
`compare <baseline experiment> <candidate experiment>` 要求同题、同尺；至少一项改善、其他不退步、无未知指标、题目全部完成才给 accepted。smoke 不参与验收。开发/回归新闻分组固定，优化选择只读开发结果，最终另查回归子集；已看过的结果不能再称盲留出。用户为 O1 明确追加目标：precision、recall 必须同时 >90%；O2 目标为 MAE <3，并同时报告 Spearman（尚无绝对门槛）。`accepted` 仅代表相对改善，不等于达到绝对目标。2026-09-21 用户为 O3 六类分类追加每类 precision、recall 均≥90%的目标，不能用 overall accuracy 或平均值替代十二项逐类判据；O3 其他字段及 O4 沿各自已有要求，不外推此门槛，不声称统计显著或已全面拟合。
