...
2026-09-20 清理后，以下迁移示例的旧源目录已移到独立归档根；要重现历史操作，先按[资产说明](../assets.md#活动题库与历史归档2026-09-20)转换旧源路径。日常扩题继续使用上表当前活动入口。评分、富化 v1 的目录链接不得当作可清理缓存；其实体和共享证据均在归档树。新 vN 输出到活动根，不输出到链接解析后的归档根。
