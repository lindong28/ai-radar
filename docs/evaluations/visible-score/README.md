...
2026-09-21 用户决定暂停评分优化，先推进 O3 网站 category / tags；之后再检验我方预测字段对评分的增益。当前收尾与恢复入口以 [status 顶部](status.md#2026-09-21-收尾评分优化暂停)为准，下方说明保留已有计算能力。
...
| 槽位 | 设计与当前能力 |
| --- | --- |
| ① 优化对象 | reason-first 六维 scorer＋固定普通展示公式为历史基线；作者结构启发的五维判断＋代码组合已作离线研究，当前暂停优化。`--mode five`、五维校验及显式百分比组合已实现；具体维度/公式仍是研究假设，不冒充原作者定义。已测[三维语义候选](semantic-design.md)、直接总分与数值校准保留为历史对照；生产未替换。 |
