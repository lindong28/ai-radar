...
2026-09-21 用户裁定：目标是拟合 AIHOT 可见分，优先采用原作者的设计，而不是自行重新定义维度。此决定替代“三维 S2 作为下一轮研发起点”的路线，不否定已完成实验的读数，也不意味着已经实现或验证新的五维候选。
...
复用现有题库、MAE、reason-first 与开发／独立回归流程；三维 S2、旧六维 B0 仅作为已测实现对照。当前新增的是路线及证据边界记录，没有新五维 prompt、代码或模型评测成绩。原作者的具体维度、刻度与公式仍是待研究部分，不将“架构相似”当作“已还原”。
