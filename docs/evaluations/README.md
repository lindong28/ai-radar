...
- 文本判官新调用经 llm-gateway，明确请求 logical model `deepseek-v4-flash`，实际 native model/provider 由响应身份记录，每字段 0/1/2 分。12 个用户确认样本（每字段4个），开发6、独立验证6；验证全一致才在该判官身份下采信。设计认可不等于已经有用户票；小校验不能证明广泛可靠性。旧冻结运行中的 `deepseek-v4-flash-ga-260731` 身份不改写，新通道不能沿用旧判官校验凭证。接入前置条件见 [Gateway 运维](../operations/llm-gateway.md)。
