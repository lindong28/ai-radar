...
2026-09-20 用户已选定 **C11 reason-first＋既有 `hn100-standalone-body-v1` 规则**，当前本地源码默认采用该组合，尚未 push 或部署。模型先输出简短 `reason`，再输出判断；最终准入为模型判断与原始输入规则的合取。模型原判断与规则拒绝原因分别保存，不依赖新增引用正文、网络请求或 Pro。具体输入条件、默认评测与显式候选的区别见[执行入口](../../../evals/news-admission/aihot-observed-membership/README.md#当前默认组合与回放)，采纳依据和验证边界见[当前状态](status.md#当前采用)。

## L1 计算逻辑

| 槽位 | 设计与当前能力 |
| --- | --- |
| ① 优化对象 | 当前默认为 C11 reason-first 模型＋既有原始输入规则的最终准入；纯模型候选须显式区分，不混入后置 relevance。 |
