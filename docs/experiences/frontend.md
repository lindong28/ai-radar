...
## 2026-08-20 后记：上一条里 `app.py:47,50` 的行号与「只对这两条路径」都已过时

- Problem: 上一条（2026-08-17 静态资源漏 bump）在讲漏 bump 后各页面的缓存尾巴时写了「`src/airadar/web/app.py:47,50` 只对这两条路径发 `max-age=90, stale-while-revalidate=30`」。两处都已漂移：行号不再对（该常量与路径表现在约在 `app.py:52` 与 `55–61`），而路径也不止两条——`_PUBLIC_PAGINATION_QUERY_KEYS` 当前有五条：`/`、`/wechat`、`/api/v1/curated`、`/api/v1/hot`、`/api/v1/wechat`（`/api/v1/hot` 是随 ADR-060 热点缓存加进来的）。
- Solution: 判「某条路径有没有公共缓存尾巴」时读 `app.py` 里的 `_PUBLIC_PAGINATION_QUERY_KEYS` 本身，不要引用行号；页面路由的权威清单是 [architecture.md §页面路由](../architecture.md#页面路由)。上一条的**结论不变**（补 bump + 重新部署，各档尾巴长度的量级也不变），只是那句括注的枚举与坐标不再准。
- Applies when: 引用源码行号写文档时——行号是本项目文档里最先烂掉的一类事实。指名符号（常量名、函数名）grep 得到，行号只会把读者带到别处。
