...
## 生产闸只长在服务器上时，本地的"全绿"什么也不保证（2026-09-10）

一次改动把一份小的度量标注文件 track 到 `data/` 下（`.gitignore` 开了个 `!` 例外）。本地这些全绿：
ruff、mypy、2790 个测试、`create-commit` 的导出树执行检查、`git check-ignore` 双向对照。
`git push` 也报成功。**而部署被拒了**——`data/` 整个是 runtime-owned，部署在动树之前就拒绝这类 commit，
生产停在旧代码上，只有健康检查会 page。

**这不是"闸太严"**：`checkout-index -f` 真会覆盖 git 恢复不了的线上状态，拒绝是对的。问题在于
**那条判据的唯一实现在服务器上**，而本地的每一道检查按其定义都看不见它——它们查的是代码正确性、
路径归属、忽略规则，没有一道查"这棵树能不能被部署接受"。

**可迁移的做法**：服务器侧的部署前置条件，凡判据是**纯函数、且输入在本地也拿得到**的（这里是
「路径字符串 → 是否 runtime-owned」），就在本地加一个测试**拿本仓真实的文件清单**去过它。
判据本身已经有测试不够——那次就有两个，一个测谓词、一个测合成 commit，**都不读这个仓自己的树**。
`git ls-files | 过一遍谓词` 一行的事，而它能挡住的是一次"推成功了、生产没更新"。
