...
| [20260919-d62b](adr/20260919-d62b-test-prefilter-ai-capabilities.md) | Decision | 离线测试AI设备实质能力及来源上下文；双90累计6000次 |
...
[新闻准入时间标注修订草案](evaluations/news-admission/time-label-design.md)：待用户审核；区分冻结批次已观察成员与 ±12h 时间匹配，不改当前题库或 gold。出版边界见 [20260919-9365](adr/20260919-9365-publish-admission-time-label-proposal.md)。
...
[逐对象建题与扩展](evaluations/benchmarks/object-datasets.md) 包含合并去重、有效性重验、命名迁移与复用命令。当前说明：[新闻准入](evaluations/news-admission/aihot-prefilter/v1/README.md)、[可见评分](evaluations/visible-score/aihot-score-pointwise/v1/README.md)、[字段富化](evaluations/content-enrichment/aihot-enrichment-fields/v1/README.md)、[局部精选阈值](evaluations/featured-members/aihot-featured-threshold/v1/README.md)。契约不同拆 benchmark，契约相同用 vN；评分/富化可用 AIHOT 原文，准入/精选不因此扩充。原 objects/ 与规则名目录的有效内容已迁入这些单一入口，旧数据与成绩保持历史身份。
...
| [对象、指标与建题](references/aihot-eval-foundations/objects-and-metrics.md) | 四对象的输入输出、最小指标、分母、从 raw 建题方法与共用三层约束；不另设最终页面评测对象 |
...
| [20260915-1cc7](adr/20260915-1cc7-retain-continuous-prefilter-inputs.md) | 持续原始输入与 AIHOT 缺窗补收的本地实现决定 |
