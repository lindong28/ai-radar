...
⚠️ **reviewer 把这读成"首页主面退化成 all-time greatest hits"，caller 初稿照抄了，两者都不成立**——那是单轮
`output_curated_ids` 的读数，而首页是本 ADR 交界面里那个**跨轮并集按 `published_at` 取前 40**（ADR-006）：
老条目进了并集却排不进第 1 页。实测退化期间生产首页第 1 页的跨度仍是
`2026-09-11T21:59:40Z` → `2026-09-13T03:11:15Z`。真正的用户可见形态是**首页不再往前走**：退化窗内
新入库的 2203 条中只有 1 条被选进精选（恢复后同一口径 26/2411）。老条目落在归档更深的页上，那是下面
「残留」那一段说的污染。它不报错、不告警。
