...
## 部署结果与生产验收（2026-08-20）

按 ADR-042 制作隔离部署 commit `D` = `c6d12e15717326876a71627d554b30c8ded98bff`，父节点即当时的 `tencent/main`（`5039988f`），只含本任务 24 个文件——直推本地 `main` 会连带发布 7 个属于其它任务的 commit。在该生产父链上：1625 passed / 0 failed、ruff 与 mypy 干净、资产版本自洽、503 闸变异对照转红、`admin edgeone check` exit 0。用户授权后 fast-forward push，post-receive 完成 slot 8000 部署，`.deployed-sha` 与 `D` 一致。

**origin 本机读数（`127.0.0.1:8000`，绕开 EdgeOne）**

| 入口 | 改前 | 改后 | 阈值 | 结论 |
|---|---|---|---|---|
| `/api/v1/hot?limit=2`（连打 3 次） | 14.29 / 14.35 / 14.37 s | 2.4 / 2.2 / 2.0 ms | < 100ms、极差 < 50ms | 通过（极差 0.4ms） |
| `/hot` 整页 | 14.9 s（公网） | 2.6 / 2.5 ms | < 1.75 s | 通过 |
| `/` 默认视图 | 1.447 / 1.453 / 1.463 s | 1.301 / 1.309 / 1.314 s | < 1.75 s | 通过（快于基线） |
| 首页 HTML 含 SSR 热点块 | — | `data-loaded="true"` + 2 条 `hot-topics-link` | 命中即直出 | 通过 |

**公网读数（经 EdgeOne）**：`/api/v1/hot` 0.35 / 0.13 / 0.16 s、`/hot` 0.14 / 0.16 s、`/` 1.48 s，首页 HTML 已含 SSR 热点块。残余耗时是跨洋 RTT，不是计算。

**未在生产观察到的一项**：冷态那一行（重启后立刻打应得 503 且 < 2.2s）——keeper 在第一个请求到达之前就已把缓存填好，那个窗口没被观察到。它只在本地验证过，不能算作生产已核实。

**剩余项**：生产历史与本地 `main` 尚未清账。`D` 是复放而非本地 `main` 上那个 commit，故 `git merge-base --is-ancestor D main` 不成立。按 ADR-042 须等并发写入者释放后另取整合许可；在此之前保留 `ai-radar-worktrees/deploy-hot-cache-20260820`，不得把生产部署成功表述为本地 `main` 已完成清账。
