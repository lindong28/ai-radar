...
- **第 2 轮（未执行，2026-09-05）**：原定每组 n≥100 复跑；09-05 22:11 只读探针 gained 83 / lost 92，未达标。同日用户重定 program 20260820-content-align 的方向：以 AIHOT 历史数据为带参考输出的评测题、以与 AIHOT 的自动一致性指标为优化目标，不再区分编辑口味与质量。判官「值得读」验收随之弃用、不再复跑；配额保持开启，本 ADR 的决策与回退机制不变。后续评测体系重新设计的交接材料在 `handoffs/aihot-fit-llm-eval-system-handoff-20260905.md`（本地未跟踪路径）。
