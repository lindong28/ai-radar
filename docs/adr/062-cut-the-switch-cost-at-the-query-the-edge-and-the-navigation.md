...
用户在三者中显式选 C。外部决策评审推荐"A+B 先行、客户端切换后置到有真实读数再定"，理由是其增量收益当时无读数；该推荐连同理由摆给用户后，用户仍选同批实施。**这是一次显式 waiver，记录在此**：客户端切换在**生产**上的增量收益在落笔时没有真实浏览器读数。**该缺口已于 2026-08-21 结清**——见 Consequences 的「部署后补测」节与「未验证」第 2 条：实测速度收益约等于零、反复来回时为负，评审当初那条推荐因此被读数支持，而用户在看过读数后仍裁决保留（理由是三条非速度属性）。读到本行的人不要再把它当作开口的缺口。
...
据此，本 ADR 落笔时不对 D5 的增量收益作任何量化声明。playwright 建立的是结构事实：切换不重载文档、不丢滚动位置、侧栏状态跟随、失败回退真导航、监听不随轮次累积。

### 2026-08-21 部署后补测：D5 的速度收益实测约等于零，常见情形下为负

上线后对真实站点重测（同一浏览器会话、两臂同为墙钟、同一完成判据；仪器阴性对照 `/about` 中位 145 ms，非此前被系统代理污染的 1012 ms）：

| 情形 | 整页导航 | hover 后客户端切换 |
| --- | --- | --- |
| 首次切换（全新 context、空浏览器缓存、普通 URL 走边缘） | 72.3 ms | 67.6 ms |
| 反复来回（90 秒内，浏览器已缓存两页） | **17.7 ms** | 59.7 ms |

首次切换的差异落在噪声内（样本区间重叠）。**反复来回时原生导航快 3.4×**——而"来回点"正是本决策 Context 里用户描述的那个行为。

成因是上面第 119 行那句的完整兑现：`max-age=90` 让浏览器缓存了两个文档，原生导航于是不付网络往返、只剩文档重建；而客户端切换仍要 fetch（即便命中预取缓存）、DOMParser 解析、替换 DOM、重跑 initializer，其自身开销高于它省下的重建。

**Options 节里"客户端切换是速度上补不上的那部分"这一推断，据此被证伪。** 把 1.55 s 降到 0.065 s 的是 D1–D4；D5 没有带来可测的速度收益。它仍然提供的是非速度属性：不白屏、后退恢复滚动位置、侧栏状态跟随——这些有 playwright 断言，但它们是体验属性而非延迟。

一处方法学教训：第一次补测把 cache-busted 的 `1382.6 ms` 当成了冷启动读数。那不公平——`?cb=` 不在 `_PUBLIC_PAGINATION_QUERY_KEYS` 里，origin 回 `private, no-store`，量到的是绕过边缘的 origin 渲染，而真实首访走边缘。上表的冷启动改用全新 context + 普通 URL 重测。

**用户裁决（2026-08-21）：保留现状。** 三个选项——去掉 swap 只留预取（当时的推荐项，理由是它保住唯一有正向读数的那半、同时消掉整套只为 swap 存在的生命周期风险面）、保留、整个撤掉——连同上面的读数一并摆出后，用户选择保留。据此 D5 继续留在代码里，其正当性来自那三条非速度属性，**不再来自任何速度声明**。后续读者不要把上面那张表读成"D5 待撤销"。

保留意味着两项已知成本继续存在：反复来回时每次多约 42 ms，以及每次跨路由 hover 多发一份完整 SSR HTML 的请求（未点击者零收益，外部评审记为 MEDIUM，见「未验证」第 6 条）。
...
2. ~~客户端切换在生产上的增量收益无真实读数~~ —— **2026-08-21 已补测**，见 Consequences 的「部署后补测」节：速度收益约等于零，反复来回时为负。此项不再是缺口，而是一条推翻了 Options 推断的结论。
3. **AIHOT 的客户端切换未被直接观测**。它对 agent 返回 `{"error":"blocked","code":567}`，headless 进不去；架构判断来自其 HTML 里的 Next.js App Router chunk 路径与侧栏形态。
4. ~~EdgeOne 规则尚未修改~~ —— 2026-08-21 已在控制台改毕并从公网验证（`/` 与 `/all` 均 MISS→HIT）。**CAM 密钥确认没有 `ModifyL7AccRule`**：API 调用被 `AuthFailure.UnauthorizedOperation` 拒绝，拒绝发生在任何变更之前，读回确认线上未被改动。详见 `docs/operations/services.md` 的凭据节。
