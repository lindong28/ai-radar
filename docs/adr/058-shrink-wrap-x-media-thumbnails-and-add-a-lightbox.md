...
- **未覆盖轴**：125/150/200% 缩放档、断点两侧取点（959/960/961）、以及 Windows/Linux 上 Ctrl+左键的语义（本机 macOS，不推断）。dark 主题已覆盖。
...
- **Firefox 未覆盖**（Playwright 的 firefox 二进制两次下载失败，非结论）。已覆盖 **Chromium 147 与 Playwright 的 WebKit 26.4 构建**——后者与 Safari 同源但是**定制构建，不等于在真机 Safari / iOS 上验证过**，那两个平台仍未覆盖：两个引擎在 {1440×900 light, 390×844 dark} 上逐项相同——布局违例 0、同卡「全部失败 vs 无媒体」delta = 0、手势矩阵全通过、抛错回滚无残留、Esc 后残留干净且焦点归还；`inert` / `:has()` / `CSS.escape` 三项特性支持在两个引擎上均为 true。
