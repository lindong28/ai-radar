...
**达标线仍不写。** 定它之前要先扣掉 ISSUE-FIT-01 那个 schema 天花板：本轮零成本量到，判官 closeness 与「我方句数 − 参考句数」的差值单调负相关（差 0 → 80.0，差 1 → 52.5，差 2 → 56.2，差 3 → 46.8，差 4 → 40.0；Pearson r = −0.307，n=37），而我站 schema 强制 3–5 句、参考里 30/37 是 1–2 句，故该差值**结构性地**落在 2–3。在这个刻度上定阈值会把一个 prompt 改不动的分量焊进去。

**本轮基线的实际代价**（ISSUE-FIT-07 的实测值，此前只有估算）：三次 run 的 item 在 `data/llm_usage.db` 留下 **970 行**（enrich 344 / prefilter 313 / score 313），占当日该表 4066 行的 **23.9%**，且与生产流水不可区分。跑全量 2741 题会把这个数推到约 8200 行。
