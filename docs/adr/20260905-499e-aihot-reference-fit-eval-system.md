...
### 分类修完后，池子本身仍离 AIHOT 0.272——那是信源问题（2026-09-08 初步读数）

上一节说「我方的地板取决于我方池子的构成，而那个数要等重算排空」。重算跑出 1527 行新标签之后，可以给一个**带限制的初步读数**：

| 口径 | n | 对 AIHOT 精选构成的 TV |
|---|---|---|
| 我方池·**旧** prompt 标签 | 7020 | 0.361 |
| 我方池·**新** prompt 标签 | 1527 | **0.272** |
| AIHOT 自精选（目标） | 78 | — |

分类修复把池子的构成推近了 **0.089**，但**在任何排序发生之前，池子仍差 0.272**：

| 类别 | AIHOT 精选 | 我方池（新标签） | 差 |
|---|---|---|---|
| industry | 12.8% | **33.7%** | **+20.9pp** |
| model | 24.4% | **12.2%** | **−12.2pp** |
| tutorial | 32.1% | 22.5% | −9.6pp |
| paper | 10.3% | 4.8% | −5.5pp |
| product | 20.5% | 26.8% | +6.3pp |

**所以「改排序还是改信源」这个二选一有答案了：改信源。** 我方信源产出的 industry 是 AIHOT 的 2.6 倍，model 只有它的一半——排序再准也变不出池子里没有的东西。这与 ADR-047 的信源覆盖线是同一件事。

**两条已知限制，别越过它们读**：① 这 1527 行是**最近被重算的那一批**，按 `fetched_at` 排序取的，不是池子的随机样本——重算排空之后要重测；② 比较的对象是 AIHOT 的**精选**构成而不是它的**流**构成，而上一节测得 AIHOT 挑得不算挑（各类精选率 2.0–2.6%，model 约 2.4 倍），所以两者接近但不相等，这一层误差没有单独扣掉。
