...
**上游只有两个可能，而现在没有读数能分开它们**：抓来了但被 prefilter 判非 AI 扔掉（→ prefilter 是首要瓶颈），或者根本没抓（→ 信源覆盖，AIHOT 有我们没有的源）。两条的下一步几乎不重叠。分开它们要一批人工标注的漏判读数——见 `prefilter-negatives` 那一节末尾的方向限制：recall 侧这份数据构造不出来，必须人标。
