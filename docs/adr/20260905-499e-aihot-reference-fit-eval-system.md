...
**一条只看标题的正则，在一致率与构成上双双压过读全文的生产分类器。** 这否定了「这个任务本身很难 / 模型能力不足」——它的 spec 把它指反了。同时注意规则自己也没解决问题：它把 68.8% 的条目倒进 tutorial（AIHOT 是 35.7%），是我方 LLM 那个错误的镜像。所以**两者都不能直接用**，规则的价值是给出了下一版 prompt 该编码的语义，以及一条 0.547 的、必须被超过的对照线。
...
**下一版 prompt 的形状约束**（按 `prompt-writing-guidelines.md`）：§2「描述目标而非流程」与 §4 反模式表点名的「拆成很多条件分支」「否定式范围排除」——现有 prompt 两条都犯了。所以改法不是往漏斗里再加分支或加例子（§1、§4「加入失败用例的示例」），而是把五类各写成**正向定义**，并去掉那条把 tutorial 排到最后的判定顺序。验收判据是同一批 1149 条上一致率 > 0.547（对照线是那条只读标题的规则，不是现状 0.509）且构成 TV < 0.319。
