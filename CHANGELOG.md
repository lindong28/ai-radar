...
## 2026-09-08（文章分类换了一套判据，并修掉一个让分类改动三个月到不了站上的机制缺陷）

- **站上大部分文章的分类，一直来自三个月前那版判据。** 分类由 enrich 阶段的提示词决定，而它换新之后，已经分过类的文章**永不重算**——重算与否看一个手工维护的版本戳，那个戳从 5 月起就没动过，提示词却在 9 月 2 日到 7 日之间改了四次。9 月 8 日实测：候选池里只有 **1.8%** 的分类出自当时生产在跑的那版提示词，**83.2%** 出自 5 月那版。现在版本戳改为从提示词本身（连同标签词表与类别清单）算出来，改了就自动变，不再依赖有人记得去改；已分过类的文章因此重新进入队列，随后的定时任务逐轮重算。
- **分类判据本身也重写了。** 此前 `industry` 的判据是「主语是不是一家具名公司或机构」，而 AI 内容几乎条条点名公司，于是它成了倾倒桶（实测占我方 50.2%，参照站点只有 14.3%）；`tutorial` 被写成排在最后的兜底类，实测只兜到 1.9%，而参照站点它是最大的一类（35.3%）。新判据按「这条内容本身在讲什么」分：`industry` 要求**确实发生了一件可指认的商业或制度事件**（钱动了、所有权或高管人事变了、监管司法落地了），第三方对模型或产品的评测与解读归 `tutorial`。同一批 298 篇文章上，与参照站点的构成差距（总变差）从 **0.144 降到 0.067**，配对 bootstrap 95% 区间 [-0.107, -0.027]。
- **重算期间新文章不会被挤掉。** 版本戳一动，24 小时窗口内的候选从 21 条涨到近 3000 条，而每轮只处理 40 条；此前的排序会让新到的文章排在几千条重算之后（实测第一条新文章排第 303 位，末条第 2440 位），它们会在轮到之前滑出窗口、永远拿不到中文标题与摘要，也不会出现在任何分类筛选里。现在从未分过类的文章永远优先。
- **你会看到的另一处变化：摘要变短了**，中位数从 147 字到 80 字。这不是这次改的——是 9 月 6/7 日那次「只写最核心的那件事，写完即止，不用篇幅解释信息为什么少」的改写，随这次版本戳移动才终于作用到存量文章上。**短是有意的**：提示词里原先还留着一句「目标 100-160 字」，与上面那条要求互相矛盾，已经删掉，现在不设长度下限也不设目标长度。硬性上限仍是 400 字。

## 2026-09-08（pipeline 停跑告警不再把「出网不可用」说成「疑似卡死/僵尸锁」）

- **同一个误导文案今天出现了第二次。** A2 的心跳支路在超过 120 分钟没有成功 pipeline 时 page，此前只会写「最近成功 pipeline 已超过 N 分钟（期间连续 SKIP n 次，疑似卡死/僵尸锁）」，处置指向「检查 pipeline 锁和长任务」。9 月 8 日 11:00–14:30 十五轮全部停在 `egress preflight FAIL`（零摄入 3 小时 32 分），根因是本机出网模式掉回 clash、而应用钉死 domain-routing 策略并 fail-closed；A2 六次 page 逐字都在说锁。9 月 4 日那次是同一个形态。
- **现在 A2 会读那一行原因再下判断。** pipeline 每轮都会打印 `egress-preflight status=… reason=…`，此前无人解析它——那行 `=== egress preflight FAIL ===` 标记的阶段名里带空格，恰好被阶段解析的正则排除在外，所以一个被出网挡住的轮次在告警侧解析出来是**零个阶段**。现在 A2 会看**最近一个非 SKIP 轮**的那一行：它报告非 healthy 时，正文写「最近一轮出网 preflight status=… reason=…」，处置改为跑 `check-proxy-status --format=kv`（preflight 自己跑的就是它）并指向 runbook 里那一节。只看最近一个非 SKIP 轮、不往更早回捡，是因为「取到锁后挂起、之后每轮都 SKIP」时往回捡会把一次真的僵尸锁归因给出网，并删掉识别僵尸锁唯一的那个 SKIP 计数；那一轮没打印 preflight 行就没有读数，文案照旧回落。**改写只在心跳支路生效**：A2 也会因阶段错误率与 P95 开火，那两条支路仍然指向阶段日志，不会因为出网碰巧不健康就被改写。出网正常、或没有任何轮次跑到 preflight 时，文案与处置都保持原样。
- **顺带撤掉一个恒为噪声的计数。** 那句 SKIP 次数统计的是「距上次成功以来的 SKIP 轮数」，而这次事故里 SKIP 轮是 **0**——十五轮全都取到了锁、跑到 preflight 才失败，它报的 2 来自事故窗口之外。归因到出网时不再并列展示它。
- **这不是新的触发条件**，只改一条已经在 page 的告警说什么、让人去查哪里；A2 何时开火不变。（`/api/v1/admin/metrics` 的 `pipeline.latest_run` / `pipeline.recent_runs` 因此多一个 `egress_preflight` 键，值为 `{status, reason}` 或 `null`；该端点由 Cloudflare Access 把守，既有消费者按名读键、不受影响。）

## 2026-09-07（薄来源的文章不再因为推荐理由太短而整篇丢掉）

- **修掉一个上线一天的回归。** 9 月 6 日那次"摘要变短"的改动里有一句"薄来源不要用篇幅解释信息为什么少"，它同时管住了摘要和推荐理由；而推荐理由另有一条 35 字的硬下限。两条要求互斥，于是模型对薄来源写出的理由稳定落在 18 个字，**整条内容被拒收**——摘要、标签、分类一并丢掉。上线后 9 小时内累计 77 篇，其中 7 日 0 点到 2 点这三小时的失败率达到 44%、65%、43%（平常约 3–6%），而且**重试也救不回来**：这 77 篇全部没有产出，一篇都没进网站。现在推荐理由不再适用"最短"这条要求，提示词里也写清了写太短的后果，并换了一个长度合规的范例。
- **另修一处提示词与实际校验不符的地方**：标签下限 9 月 6 日已从 2 放宽到 1，提示词却仍写着"标签必须 2-4 个，违规会被拒绝"。它不会造成拒收，所以一直没有报错——只是让模型继续凑标签，而那正是放宽下限要消除的行为。
- 这次改动**只改提示词、不改校验规则**，已入库文章不受影响；此前丢掉的 77 篇需要一次单独的补跑才能回来，尚未执行。

## 2026-09-06（文章排序改为拟合参照站点；标签只剩一个也不再丢掉整篇）

- **网站上文章的先后顺序换了一套依据。** 此前的权重是 relevance 0.10、density 0.40、recency 0.30、authority 0.10、engineering 0.10，再按来源分层乘一个系数（T1 ×1.25、T2 ×0.75）。实测参照站点给这三层的均分是 T1 44.7、T1.5 39.8、**T2 51.7**——我们的系数方向是反的。新的排序依据在 2741 篇配对文章上拟合出来：信息密度 0.40、来源权威度 0.10、事件重要性 0.50，来源分层系数取消。与参照站点打分的相关度从 0.45 提到 0.63（对半拆分、留出验证、五个随机种子给出同一个向量）。你会看到的变化：X/Twitter 属于 T1.5（系数 1.0），此前是 T1 被抬高 25%、T2 被压低 25%，取消系数后 X 在候选里的占比实测从 77% 降到 59%；此外，"这件事在 AI 领域有多大"开始主导排序。
- **打分新增一个"事件重要性"维度。** 此前五个维度问的都是这条内容本身如何（相关性、信息密度、新鲜度、来源权威度、工程可用性），没有一个问它讲的那件事有多大。而参照站点打分最高的 18 条里有 14 条是同一个收购案，其中一条正文只有一个链接、照样接近满分。加上这个维度后，同一批文章的拟合度从 0.53 升到 0.59——**这一段提升还不是来自新维度参与加权**，而是来自提示词里多了这一问之后，原来那五个分本身给得不一样了（那次测量用的仍是不含它的旧权重）。把它真正加进权重是后面那条排序改动。这个新维度单独一项就比原来任何一个维度都更能预测参照站点的评分。
- **AI 相关性与新鲜度在排序里权重归零**，不是因为它们不重要，而是因为已经有别的环节在管：AI 相关性由前置筛选阶段把关，新鲜度由精选的 48 小时窗口把关（且只保留最新一天的候选）。同一件事在排序里再算一次没有增量。
- **打分有了自己的规则版本号**（`2026-09-06.r1`，与 prefilter / 旧 enrich 共用的 `2026-05-13.r1` 分开）。此前三者共用一个号，于是打分行为变了却无法单独升版——改动前后写下的行在库里**声称同一个版本**、事后分不出，而且打分器会跳过"已经在当前版本下打过分"的条目，导致一次不升版的打分改动**永远无法作用到存量文章**。升版之后，下一次定时任务会重打它时间窗口内的条目，更早的存量由一次显式的全量回填处理。
- **`./run.sh score` 新增 `--force`、`--workers`、`--commit-every`**，都是为了让一次存量回填真的跑得完。`--force` 重打已经打过分的条目（此前只有换 ruleset 版本号才能触发，而改提示词并不换版本号，于是一次打分改动在存量上永远不生效；那个开关当时还是从一个测试用环境变量推出来的）。`--workers` 默认 **1**——定时任务保持串行不变，并发是回填显式开的；实测串行是 2.25 秒/条，四万条要 25 小时。`--commit-every` 默认每 200 条提交一次：此前整个循环只在末尾提交一次，一次跑了两分钟被打断的运行**一行都没留下**。另新增环境变量 `AI_RADAR_SQLITE_BUSY_TIMEOUT_MS`（默认仍是 5000）：长时间批处理会反复撞上定时任务持有的写锁，5 秒不够。
- **修掉一个让上面那个变量形同虚设的老 bug**：`001_init.sql` 里写着 `PRAGMA busy_timeout=5000;`，而这个迁移每次命令调用都会重跑——于是连接上设的等待时长只活到第一条迁移语句执行为止，之后被悄悄改回 5 秒。影响不止批处理：**任何**命令在与定时任务竞争写锁时都会在启动期直接失败退出。连接级的运行时设置不该写进 schema 迁移，已移除，并在迁移跑完后重新设置一次以防将来复发。
- **只有一个有效标签的文章不再被整篇丢弃。** 此前要求至少两个受控标签，凑不够就连摘要、推荐理由、分类一起拒收——全量 2741 篇里有 78 篇撞在这条底线上，其中 52 篇是模型只给了一个标签。（同一次全量还有 3 篇是模型给的标签全部越界、36 篇是推荐理由太短，这两类本次不受影响，所以 enrich 失败预计从 117 降到 39，不是降到 0。）参照站点自己在这 2741 篇里有 2157 篇一个标签都不打，所以"至少两个"从来不是对它的贴近，而是我们自己的约定。现在一个就够；一个都没有（模型给的标签全部不在受控词表内）仍然拒收。已入库文章不受影响。

## 2026-09-06（分类、摘要、推荐理由、标签全面向参照站点对齐）

- **分类准确率从 53% 提到 71%**。此前给模型的只是五个类名、没有任何定义，它按字面把「tutorial」理解成教程，于是几乎不用这一类——而参照站点把它当兜底类，占三成以上。现在五类的边界写进了提示词，是从 2571 条配对样本反推出来的。最直接的变化：观点、讨论、见闻类内容不再被扫进「行业」。
- **摘要变短，从取舍策略改起**。参照站点写一到两句、中位 131 字，只讲最核心那件事；我们此前被要求「覆盖不同信息点」并硬性写满三句，中位 183 字。现在改为取一点写透，句数下限从 3 放宽到 1，中位长度降到 119 字。薄来源不再用整段文字解释「信息为什么少」。已入库文章不受影响。
- **推荐理由改成两段式**：先说来源给出了哪类内容，再说读者能据此判断什么；不再写「但」「然而」这类转折与对冲，也不再出现「适合」。开头是否用具名主体、结尾是否写读者落点，都按内容本身决定，不套固定模板。
- **标签改打技术主题**。同一条 Claude Code 用量新闻，参照站点打「编码」，我们此前打「产品更新」——现在优先打内容讲的是什么技术，事件类型标签只在没有技术主题时才用；两个标签够用时不再凑第三第四个。

## 2026-09-06（标签越界不再让整篇文章丢掉摘要和推荐理由）

- 一篇文章的标签里只要出现一个受控词表外的词，此前整条 enrich 输出都会被拒——摘要、推荐理由、分类一起没有。全量实测 2741 篇里有 117 篇因此空着，且几乎都只越界一个词，而越界的绝大多数是公司名（NVIDIA、腾讯、小米、Amazon）。现在越界的那个词被丢掉，其余标签与摘要、理由、分类照常保留；丢完后有效标签不足两个时才仍然拒收。300 篇的复测里 enrich 失败从 16 降到 8，越界这一类归零。
- 标签词表按参照站点全面对齐：新增「语音」「政策/监管」，移除「DeepMind」「GitHub」「RAG」「arXiv」「搜索」——参照站点在我们持有的 584 篇参考里从未使用后面这五个。`arxiv.org` 来源的文章此后标为「论文/研究」（参照站点用的正是这个），`github.com` 来源不再自动带上「GitHub」。已入库文章的历史标签不受影响，只影响此后新处理的文章。
- 标签数量不合规不再丢掉整篇。模型被要求写 2-4 个标签，写多了或写少了此前都会让整条 enrich 输出被拒。全量实测 2741 篇里有 75 篇栽在这一条上，是最大的一类失败。修好后再测才看清方向：**81 条失败里 0 条是写多了，52 条只写了 1 个标签**——过去两个方向共用同一条错误消息，且被拒时模型的原始标签没有落盘，所以先前无从分辨（现在会落盘了）。所以真正起作用的是下面那条底线放宽，而不是截断。截断仍然保留，并改在按词表过滤和去重之后做，否则排在第五位的合规标签会先被位置截掉。
...
- 表现为「间歇」是假象：该探测只在特定分支才调用 `lsof`，所以同一个确定性缺失时而命中、时而不命中。
...
- `deploy/wechat2rss/healthcheck.sh` 在健康时清除全部告警去重 key，并在「告警中 → 健康」的转换上发一条恢复通知。此前去重 key 一旦发过就永不清除，08-17 演练失败分支留下的签名把这次停机的 335 次告警全部静默压掉，运维零感知。

## 2026-09-03（精选按来源形态配额，降低单一形态与单一来源集中度）

- 精选首页的新一轮内容默认把 X 推文占比限制在约两成，并把单一来源限制在约 7.5%（默认 40 条的一轮最多 3 条）；名额会继续从其他来源形态的候选中补齐，候选不足时不突破上限。可用 `AI_RADAR_CURATE_SOURCE_QUOTA=off` 恢复后续轮的旧选择行为，也可用 `admin curate rollback-quota` 定向撤除配额独有的历史入档条目。

## 2026-09-03（enrich 每轮限批 40 条，积压分轮排空不再占锁数小时）

- `pipeline.sh` 的 `enrich` 阶段现在每轮最多处理 40 条（与 `interpret --limit 30` 同一形态）。此前切换生成链版本后，24 小时窗口内的全部条目一次性变成新候选（2026-09-02 实测 4295 条、每条约 6 秒），单轮占住 pipeline 锁 5 小时以上，其间抓取、精选与解读全部跳过；现在积压按轮排空，抓取与精选不再被 enrich 积压卡住。
- 因输出不合规（受控词表 / schema 校验）而 enrich 失败的条目 24 小时内不再重试（此前只要信源还列着它，每轮都会重来）：新条目优先拿到每轮名额，这类失败项每天最多重试一次；供应商瞬时故障（连接错误、5xx、限流）不退避，下一轮照常重试；`--item-id-file` / `--curated-run` 的定向重跑不受退避影响。
- 影响读者的表现：候选按信源最近一次列出的时间倒序处理；仍被信源列出的积压条目，其 v2 主分类/观点标注会在随后的轮次里陆续补上，已不再被列出的沿用 v1 投影展示（中文标题/摘要本就来自 v1，不受影响）。

## 2026-09-02（内容 v2：主分类与观点标注接入展示层，v2 生成链默认关闭待启用）

- 每条内容现在带 `primary_category`（model/product/research/engineering/business/policy/tutorial 等主分类）与 `is_opinion`（观点标注）：v2 生成链产出的条目为模型判定值，存量 v1 条目由既有标签投影给出（`classification_projection_authority=legacy_v1`），前端与 API 读者无需区分来源。
- 每日报告页的分区归类优先按 `primary_category` 映射（教程归入实践分区），"今日新模型"计数优先按主分类判定；观点类文章标题旁新增"观点"角标。字段缺失时维持原有按标签归类的行为不变。
- 新增 v2 内容生成链（AIHOT 对标的分类/标签词表/摘要与推荐理由 prompt，ruleset 版本戳 r2 与 v1 数据天然隔离），经 `./run.sh enrich --v2` 或 `AI_RADAR_ENRICH_V2=1` 启用；默认仍走 v1，未启用时生产行为不变。

## 2026-08-31（微信搜索可按记忆词找回文章，并可手动补录本地知识库）

- `/wechat` 搜索现在把空格和标点分隔的词逐个匹配到标题、公众号、正文、摘要、标签或完整解读，保留简繁兼容；严格索引查询为零结果时，再启用空白不敏感兜底。`实测`、`评测`、`测评`、`狂测`作为受控同义组处理，但不会放宽成缺词也能命中的模糊搜索。原始作者命中仍最优先，原始标题词命中高于仅同义词命中。
- 新增 `./run.sh admin wechat-kb import`：扫描 ai-assistant Summary Agent 的版本化文章目录，只把文件、索引和向量都有效且尚未进入 AI Radar 的微信文章补入内部归档来源。`--dry-run` 只报告，`--limit` 可分批执行；每次成功导入返回审计 run id，失败运行在事务内零提交。
- 内部归档文章会出现在 `/wechat` 并参与后续微信跨源去重，但不会出现在公开信源列表、抓取调度或来源静默告警中。网站请求仍只读 `radar.db`，不在请求时访问 ai-assistant 文件系统。
