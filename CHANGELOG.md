...
- **但真正的代价不是那条假日期，是首页约 27 小时在发几个月前的内容。** 精选的 40 格里有 36 格留给"新鲜度"，而新鲜度只认**最新那一天**。一条未来戳独占了那一天，于是 36 个新鲜槽只用掉 1 个，剩下的从**全部历史**里按分数补——9 月 13 日凌晨连续多个 run 里，入选条目的最早发布时间都是 **2024-12-18**。这个退化**不报错、不告警**，页面看起来只是"有点旧"。
