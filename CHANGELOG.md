...
## 2026-09-09（精选排序按类别微调：教程多一些，论文少一些）

- **精选页会变：tutorial 类多一些、paper 类少一些。** 实测同一时刻同一批候选，构成与 AIHOT 的总变差从 **0.193 降到 0.151**。改的是排序分数上的两个类别系数（paper ×0.90、tutorial ×1.12），**不是配额**——配额会不管池子里有什么都强凑目标比例，系数只改变排序、构成仍随当天的候选自然浮动。
- **为什么只动这两类。** 先刻画了 AIHOT 自己的选法：它对类别几乎不挑（tutorial 0.90x、product 0.88x、paper 0.92x、industry 0.69x），只对 model 有 2.19x 偏好；它精选里 tutorial 多，是因为**它的流里就多**（35.7%，我们 23.8%）。对照下来我方只有 paper 一处明显反了——它被抬了 2.63 倍，而参照物是压的。
- **照抄它的系数试过，更差，所以没采用**：同样的系数作用在不同的流上得到不同的构成（35.7%×0.90=32.1%，而 23.8%×0.90=21.4%），实测总变差 0.215，比不改还差。现在这两个数是**按结果拟合出来的**，不是抄来的。
- **两处已知的过头，写在这里而不是藏起来**：这一轮里 tutorial 从 20.0% 跳到 40.0%（AIHOT 是 32.1%）、paper 从 16.7% 降到 5.7%（10.3%）。系数对当前池子偏强。
- **跨窗口复核试过，但没做成**：连着六天的日窗里，这两个系数从未让指标变差（五天改善、一天持平、最差 +0.000）——但那六天的分类分别出自**五个不同世代的分类 prompt**，而这个指标正是按分类算的，所以那更像在比五套标注口径而不是比六天。真正干净的只有 9 月 9 日一天（−0.041），9 月 8 日算半干净（−0.015）。要做实它，得在多个窗口上用同一个 prompt 重跑分类。
