...
## 2026-09-17（持续原始采集恢复修复，待启用）

- 本地新增采集有限重试、AIHOT 每小时补收启用后缺日、独立健康检查和恢复通知入口；AIHOT 补充探针不可用时记录诊断，仍可采集可用的 API/SSR。新增入口尚未安装 cron，生产仍运行原代码，本地实现审查已通过，生产启用与运行验收待完成。
- Radar 保留完整过滤前输入，不按分数、AIHOT 收录或精选结果预筛；不补启用前历史、不删除网站数据。数据发布仍被安全扫描误报阻塞，未绕过扫描。启用、发布与验证边界见[持续采集说明](docs/operations/continuous-eval-data.md#2026-09-17-持续采集恢复修复待启用)。
