...
- 前端视觉与体验对齐参考站 [AIHOT](http://aihot.virxact.com/)：建立完整的 design token 层（7 档字号、3 档行高、6 档间距、5 档边框、圆角与阴影），页头去卡片化，分类 tab 改为贯穿底线 + 2px 下划线选中态，侧栏宽度与选中态、主题三态控件顺序对齐；「精选」标注从右上角纯文本改为来源行内的琥珀色 chip（`✦ 精选`），评分独立到卡片右上；日期分组头改为「8月4日 ⌄」+ 独立的「星期二 · N 条」meta。**新增热点榜页 `/hot`**（近 48 小时完整榜单，名次四档配色，仅在有真实关联讨论时才提供信源展开），首页热点模块相应改为 2 条 + 「完整榜单 →」入口；`GET /api/v1/hot` 响应新增 `generated_at` 与逐条的 `published_at` / `fetched_at` / `event_time` / `source_kind` / `author` / `related_discussions`，相对时间基于 `event_time`（`published_at` 不可解析或晚于响应时间时回退 `fetched_at`）。**新增更新日志页 `/changelog`**（渲染仓库根 `CHANGELOG.md`）。**响应式重做**：断点从 760/1100 改为 640/960；≤960px 时侧栏与汉堡抽屉整体替换为 54px 底部 tab 栏（精选 / 全部 / 日报 / 更多），顶栏改紧凑条，分类筛选改为可横向滚动的药丸 chip 且搜索折叠为放大镜跳 `/all#search`（`/all` 保留完整搜索表单），卡片去边框全出血、条目间发丝线分隔，日期分组头改浅灰吸顶条并对今天/昨天显示相对日名，可点击元素满足 44px（紧凑控件 36px）触控目标；**新增仅移动端可达的 `/more` 页**收纳微信文章解读 / 收藏 / 关于 / 更新日志四个入口。**日报页重做**：从深色报纸风改为浅色并接入全站 token，归档栏改为按月份分组的可折叠列表，新增「今日看点」摘要卡（`N 篇报道 · 约 M 分钟`，`M = max(1, ceil(C/300))`，`C` 只计正文摘要的可见中文字符，为本站自定的估算契约），章节头改为「01 中文名 ENGLISH · N 篇」，报头的 `VOL` 与 `STORIES` 随实际日期和条目数动态生成；日期非法或晚于今天时改为回退最近一期 + 一行非警示色的内联状态文字（原为醒目警示横幅），而合法但当天无内容的日期不再回退、保留显式空状态。桌面侧栏「内容」分组新增热点榜、「更多」分组新增更新日志。所有数值取自参考站编译 CSS 的实测值并在本站 token 体系内重写，未引入任何第三方编译产物。
