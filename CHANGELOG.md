...
- 新增本地 HTML 实验图谱：展示 A0–K2 方案依赖、实际对照、整体与目标子集表现、逐题修复/回退和实际 prompt/reason，区分缺失证据与已有归因。
- 支持浏览器人评暂存与整份复制/下载；不自动修改 gold、指标或生产。生成与复用见[图谱入口](evals/content-enrichment/aihot-category-navigation/atlas/README.md)。

## 2026-09-23（分类K阶段Flash边界实验）

- 新增K1/K2离线分类提示词并实际完成七轮1,322次调用；保留宽定义的失败、收窄后的局部修复及原样重复反证。
- 同期全361题C5为80.61%、K2为78.95%，K2修15/退21且论文召回率下降，不替换C5整体对照或生产A0。全部逐类指标、原始理由、复算入口和下一研究问题见[分类状态](docs/evaluations/content-enrichment/status.md#2026-09-23k阶段单调用边界实验)；六类双90仍未达。
...
- 完成五轮1,267次Flash调用：J2全361题复核覆盖47/80个首轮错误，但184次复核仅净增1题，最终78.12%。保留路由研究机制，不替换C5或生产A0；六类双90未达，gold不变，逐类结果、成本与归因见[分类状态](docs/evaluations/content-enrichment/status.md)。

## 2026-09-22（分类I阶段条件盲复核）

- 分类评测器增加默认关闭的盲复核及仅用于复核的指导文本开关；保留旧调用行为、首轮输出及完整身份元数据。
- 完成五轮1,004次Flash调用：I2冻结361题同次首轮78.67%→最终79.22%，26次复核修6/退4，但仅覆盖15.58%的首轮错题，回归部分净退2。保留有效局部指导，不替换C5整体主方案或生产A0；六类双90未达，gold不变，详见[分类状态](docs/evaluations/content-enrichment/status.md)。

## 2026-09-22（分类H阶段局部机制组合）

- 实际实现并评测C5＋局部边界的H1/H2/H3，完成七轮1,322次Flash调用；逐轮归因、原件和复现方式已记录。
- 同期全361题C5为79.22%、H3为75.90%；保留H3的具体局部收益供结构研究，不替换整体C5或生产A0。六类双90仍未达，gold不变，详情见[分类状态](docs/evaluations/content-enrichment/status.md)。
...
- 同期C5全库80.06%、F3为79.78%。C5保留整体成绩对照，F3作为下一研究修复主分支；六类双90未达，未改生产或gold。逐类指标、原件、成本、归因及复现方式见[分类状态](docs/evaluations/content-enrichment/status.md)。

## 2026-09-22（分类E阶段保留收益与退化修复）

- 保留D2研究发现主次机制，完成E1/E2/E3三轮边界修复实验及D2全361题检查，共961次Flash调用；新prompt、reason、负结果与复用命令已留档。
- 当前整体对照C5全库79.78%，D2为78.39%；方向局部有收益但六类双90仍未达，不修改生产或gold。整体最好对照与研究修复分支分开记录，详见[分类状态](docs/evaluations/content-enrichment/status.md)。

## 2026-09-22（分类D阶段实验）

- 完成四个C5派生候选的同200题实验及C5全361题重复，共1,161次Flash调用；新候选均未取代C5，未改变生产默认或gold。
- 最新C5全库79.78%（上次80.33%），同prompt重复有25题换类，六类双90仍未达。候选prompt、复现方法、逐类指标、负结果和归因已留档，见[分类状态](docs/evaluations/content-enrichment/status.md)。
...
- 新增冻结预测补算入口，保留源成绩与来源身份、另存标准实验分区；不改分类器、题库或生产，不新增模型调用。定义与复用方法见[评测入口](evals/content-enrichment/aihot-category-navigation/README.md)。

## 2026-09-21（AIHOT 六类分类与独立评测）

- 分类枚举、v2富化提示、网页筛选与日报对齐模型、产品、行业、论文、教程、观点；保留历史标签兼容，不自动重标历史数据。主生产runner未切换、未部署。
- 新增可复用网页分类捕获、原文配题及reason-first Flash评测入口。网页六类与旧API的tip语义不同，独立建成361题的 `aihot-category-navigation/v1`，不覆盖旧题库。
- 两轮prompt优化将同开发200题准确率从69%提升至77%，但冻结候选回归76题为68.42%，低于A0的69.74%，因此不推广候选。每轮输入、输出、失败及归因见[分类状态](docs/evaluations/content-enrichment/status.md)。

## 2026-09-21（评分内部分类修订）

- 新增三个离线分类/评分对照prompt，区分具体信息、验证状态与宣传语气，保留每次reason和原始输入；不影响生产默认。
- 完成三组200题及固定分类重复：Q3相对旧P3的MAE由11.495降至9.770，但未胜单调用P1的9.110；重复标签一致率86%不等于准确率，MAE<3未达。原件、逐例归因和下一研究记录见[评分状态](docs/evaluations/visible-score/status.md)。

## 2026-09-21（编辑边界与推广识别实验）

- 增加编辑边界、虚构few-shot和两阶段识别候选；新增分类缓存复用与显式维度上限/下限实验，保留reason、实际prompt、原响应及失败attempts。
- 完成1321次Flash调用及4次缓存重放。单调用P1在同开发200／已有回归100题的MAE为9.110／10.810，同期A11为9.625／11.160，排序也改善；few-shot、两调用与条件公式未证明值得采用。MAE<3未达，生产/gold不变，见[完整结果](docs/evaluations/visible-score/status.md)。
...
- A11同开发200／已有回归100题，LAD整数MAE为9.100／11.140，同域网格9.075／11.110，原A11为9.500／11.430；MAE<3仍未达到，不新增模型调用或替换生产。权重、Spearman与复用命令见[评分状态](docs/evaluations/visible-score/status.md)。

## 2026-09-21（无示例评分与错题复核）

- 新增逐维独立评分模式及三种无示例对照；每轮200开发题，新100题A2/A11对照MAE11.470/11.430、Spearman0.6347/0.6421，未证明足以替换的优势，MAE<3仍未达到，生产未改。
- 新增20条评分错题HTML复核：原文、分数、五维、真实模型理由/prompt与agent分析分开展示，可保存草稿并一次复制整批反馈；不自动修改参考分或剔题。结果、失败恢复和人评入口见[评分状态](docs/evaluations/visible-score/status.md)。

## 2026-09-21（评分排序指标）

- 可见评分新增确定性Spearman，与MAE并列报告；同分使用平均名次，缺预测或无有效秩方差时不伪造相关性。MAE<3目标不变。
- 新增 `evaluate.py rescore`，不调用模型、不覆盖旧实验；已为20个历史非smoke、非校准运行补算双指标。最新同回归100题A7为MAE8.830/Spearman0.6681、A2为9.110/0.6705，MAE改善不等于排序改善。输入信息差和条件化评分的后续设计见[评分状态](docs/evaluations/visible-score/status.md)。

## 2026-09-21（五维评分离线研究）

- 新增可复用 `--mode five`、reason-first五维校验及显式权重组合；沿用独立题库与确定性MAE，不改生产、gold或添加后置校准。维度和权重是作者公开结构启发的研究假设，不冒充其私有公式。
- 七轮同开发200题比较后冻结12示例Flash候选，新回归100题MAE8.830，同题无示例对照9.110；目标MAE<3未达到。各轮结果、归因、示例曝光和用量见[评分状态](docs/evaluations/visible-score/status.md)。

## 2026-09-20（多维语义评分离线候选）

- 按用户选择重建影响、信息增量、实质支撑三维刻度，一次Flash调用先解释再分别打分，固定50/30/20组合；不使用零权重维度、来源乘数或后置校准。新增可复用 `--mode semantic`，生产路径未变。
- S2在固定开发200题MAE11.015，新回归200题10.160；同题旧六维11.520、旧直接评分11.095。高分区仍有落后，不称全量或最终达标；[设计与逐轮结果](docs/evaluations/visible-score/status.md#当前多维语义评分)保留归因、理由和原始调用。

## 2026-09-20（逐条可见评分评测与校准）

- 接通独立评分题库的模型运行、reason-first 归档、抽样恢复及零调用分数校准；扩充题库后可复用同一入口，校准只在开发题拟合。
- Flash 候选经固定开发200题与回归200题比较，回归MAE由六维基线12.605降至新闻重要性评分＋校准的8.360；高分端仍有压缩误差，不称全量或最终达标。生产评分未变，详见[结果与边界](docs/evaluations/visible-score/status.md)。

## 2026-09-20（采用 C11 reason-first 准入组合）

- 按用户选择，将本地默认准入切为 C11 reason-first＋既有规则：先给模型理由，再判断准入，并应用已有的 HN 热度、回复关系与标题占位正文条件。保留模型原判断与规则原因，不增加引用正文、网络或 Pro 依赖。
- 默认离线评测直接测完整组合，显式 prompt 候选默认保留纯模型语义。复用保存响应完成同一已见300题的零调用接入回放；人评优先 P92.41% / R97.81%，不代表全题库或独立留出成绩。尚未 push 或部署，详情见[采用状态](docs/evaluations/news-admission/status.md#当前采用)。

## 2026-09-20（历史准入候选的人评优先指标）

- 88 个已归档准入运行复用原预测补评分，默认指标总表与 compare 采用明确人评优先口径；模型与规则视图区分，失败运行不按成功子集美化。原题、预测、原成绩与人评原票不覆盖，零模型调用。
- 更新[候选对照](docs/evaluations/news-admission/status.md)及[查询和重建说明](docs/evaluations/human-labels.md#当前指标查询与历史补评分)。同一已见300题中，C11原版＋规则为 P93.75% / R98.54%；不宣称纯模型、全题库或线上达标。

## 2026-09-20（清退旧共享池实验档案）

- 按用户要求将旧共享池 33 份实验的运行原件和元数据移出项目至本机废纸篓，对应 85 行历史指标退出统一查询；其余 176 行及数值不变。
- 当前题库、外部共享证据、人评、88 轮标准版本实验和采集档案保留。同步历史台账与对象状态，明确旧记录已不能从项目回读，旧迁移工具不再用作健康检查；[范围与恢复边界](docs/evaluations/assets.md#原始参照历史运行与-support-分区)。

## 2026-09-20（评测资产位置与历史读取修复）

- AIHOT 原始参照从 `benchmarks/aihot` 迁至 `data/aihot-reference`；旧共享池运行与 metadata 独立归档，顶层预检和迁移材料归入标准运行分区的 support，避免与当前题库混淆。
- 旧题库与运行路径通过精确映射读取，指标索引同时覆盖历史归档。迁移保留原字节、身份、人评和成绩，不新增模型调用、不删除历史文件、不改变持续采集工作树；操作与位置见[资产说明](docs/evaluations/assets.md#原始参照历史运行与-support-分区)。

## 2026-09-20（评测入口整理）

- 新闻准入建题默认切到当前 `aihot-observed-membership`，旧 ±12h 契约须显式选择；修正文档中的旧入口和归档路径，当前扩题示例以现有版本为父集。
- 准入候选 prompt 统一放在对象级 `evals/news-admission/prompts/`，旧路径仅保留兼容链接；增加当前／历史代码与实验记录导航。未改题目、标签、prompt 内容、成绩或生产逻辑，未删除历史运行与人评。

## 2026-09-20（题库目录整理）

- 四个评测对象目录各只保留当前 benchmark，五个历史 benchmark 目录移到活动题库之外归档；当前题目、人评与实验记录不删除、不改写。评分／富化 v1 使用目录链接保留共享证据依赖，后续扩题路径不变；[位置与恢复说明](docs/evaluations/assets.md#活动题库与历史归档2026-09-20)。
...
- 保存 C11 人评原票及评审上下文，新增标签导入、按字段应用与零调用重评分工具；历史 AIHOT 标签和模型输出不改写。
- 四对象后续建题与迭代优先使用对应用户人评，判官分歧先对照人评；新增判断调用要求先输出 `reason`、再输出决策。操作与边界见[人评说明](docs/evaluations/human-labels.md)。
...
- 原始采集监督的「未收全来源」告警改为**连续 3 个采集轮次**未收到才报；一轮里失败源达到应采集数一半以上时走广度旁路立刻上报，不等去抖；广度与否进入去重身份，使全盘失败能穿透已开着的单源事故。约 160 个源里每轮总有个别失败，按单轮报使它成为本机最吵的告警。在同一份底板（803 份已完成 manifest、约 90 小时、`sources.toml` 160 个应采集源）上重放三种实现的 incident 翻转数：**基线（任何一个源漏采即报）117 次 → 只加去抖 45 次 → 现版（去抖 + 广度旁路）31 次**。多轮 outage 全部照报：2026-09-17 那次 109/160 由广度旁路在第一轮即捕获（不是靠去抖）；归档里 6 次广度事件在 5 分钟采样网格上全部可见。去重身份此前是裸布尔，导致 6 次广度事件里有 3 次**完全没有推送**——占着通道的正是 `claude_youtube`，即这次改动本要压制的那个源；现已修。计数按**采集轮次**（manifest `run_id`）推进而不是按健康检查调用：健康检查每 5 分钟跑一次而采集轮平均每 5.5 分钟落一份，约 20% 的 manifest 会被采样三次以上，按调用计数会让单轮漏采自己走到阈值、并谎称「连续三轮」。计数按源独立、恢复即清零，状态存 `<state-dir>/radar-source-streaks.json`（`{run_id, counts}`，只写当前失败的源，临时文件名带 pid 以防并发拼接）。该文件损坏、为旧格式、或 `run_id` 为空时都按「无历史」处理并自愈，不抛异常——它若抛出会被上层捕获成一句「归档健康状态无法核实」并**取代整份 Radar 检查**（陈旧、无归档、manifest 校验一起消失），再经去重变成只响一次然后长期沉默。未传 streak 路径时不做去抖，消息也相应只说「本轮未收全」。
...
- 版本判定改为逐对象的实质输入比较：仅发布时间、HTML、来源标签或标题排版不同不再排除有效题，真实正文变化和参考冲突仍保留排除依据。原始证据、旧题库、网站和采集调度不改；重建结果及后续复用入口见[库存](docs/evaluations/benchmarks/inventory.md)与[建题说明](docs/evaluations/benchmarks/object-datasets.md)。

## 2026-09-18（评分与富化接入 AIHOT 历史原文）

- 评分与内容富化的建题入口增加 `--aihot-inputs`：缺少 Radar raw 时可使用已归档的 AIHOT 原标题和绑定原文，参考分数、标签、改写标题和摘要不会作为原始输入。后续 `--base` 继承获准原文与冻结证据，继续合并、去重、按当前规则重验。
- 新闻准入不补这种仅有可见正例的样本，精选本轮不扩充；连续完整候选池的要求不因评分/富化扩题而放宽。复用命令与对象边界见[建题说明](docs/evaluations/benchmarks/object-datasets.md)，实际题量见[库存](docs/evaluations/benchmarks/inventory.md)。不改网站算法、采集调度或旧题库。

## 2026-09-18（评测题库累积扩展）

- 建题脚本增加可重复的 `--base`：合并历史 v1/v2 冻结原始输入和 AIHOT 参照，去重后按当前四对象规则重新检查有效性，可同时加入新采集数据；不直接拼旧题、不覆盖历史版本。
- 新版本记录逐题/字段新增、保留、更新、移出及原因，保留合并后的原始证据以便继续扩展。后续 Claude/Codex session 的操作、计数口径与验证命令见[建题与扩展说明](docs/evaluations/benchmarks/object-datasets.md)。网站与采集调度不变。

## 2026-09-18（逐对象独立评测题库）

- 新增离线 `scripts/build_eval_datasets.py`：按对象独立筛题，支持多份已验证 AIHOT 参照、扩大原始档案范围、独立版本与完整性校验。准入补充正例单列，富化按字段拆子集，不再要求四对象共用完整原始窗口。
- 建题规则、重建/扩展命令与适用边界见 `docs/evaluations/benchmarks/object-datasets.md`。旧全池 runner 拒绝新逐条题库，避免候选池缺失下产生误导成绩；网站算法不变。
- 修复 JSONL 读取将新闻正文 Unicode 分隔符误当记录换行的问题。

## 2026-09-17（可复用 AIHOT 四对象离线评测）

- 实现准入、可见评分、字段富化、精选成员的建题、推理缓存、确定性指标、文本判官与人工校准、固定池规则回放和同题比较；生产处理及采集调度不变。
- 首版冻结5小时、22个共同来源、1,187条未过滤新闻；O1按新闻时间±12小时建立107正例、136负例，944条待定只排出O1。题库保存本机video-eval-arena，不使用DGX或旧T5。
- 真实smoke与全池调用已归档；供应商拒答、无推荐理由参考、尚无人评校准均显式留缺，不冒充完整拟合成绩。新增数据以新版本沿用同一入口，详见[评测文档](docs/evaluations/README.md)。

## 2026-09-17（采集监督清理竞态修复，已获批启用）

- 修复子脚本退出后残留进程组清理的 macOS EPERM 竞态：有界等待全组退出，不把真实权限失败当成成功。
- 监督内部异常保存失败终态，纳入已有独立健康告警；重试开始不再清除尚未恢复的故障，超预算未收尾也会报错。已获批合入本机主树，19:58 真实 AIHOT 入口及健康检查退出 0；没有新到期日窗，不计为新日窗验收。不改变原始输入范围、来源或调度频次；现场启用状态见[持续采集说明](docs/operations/continuous-eval-data.md)。

## 2026-09-17（持续原始采集恢复修复，已安装调度）

- 经用户批准，本机已安装 AIHOT 每小时第 7 分钟采集、每 5 分钟健康检查，Radar 保留每 15 分钟调度。本地实现审查已通过；首次真实 AIHOT 入口因尚无到期日而正常退出，尚未验证新日窗或长期连续性。
- Radar 保留完整过滤前输入，不按分数、AIHOT 收录或精选结果预筛；不补历史、不删除网站数据。真实自动周期完成三次尝试并保留 raw，失败退出后下一周期正常进入抓取；YouTube RSS 间歇故障仍在，不能称全源恢复。健康检查已自动发送恢复及再次告警，未确认手机收件。安全扫描误报精确修复已生效，实际暂存数据 52 个公开游标误报归零、index 不变，未改 raw 或绕过扫描。运行及发布边界见[持续采集说明](docs/operations/continuous-eval-data.md#2026-09-17-持续采集恢复修复已安装调度)。

## 2026-09-17（AIHOT 简化评测实施说明）

- 将已认可的收录、可见评分、字段富化、精选成员四对象设计整理为 [实施入口](docs/references/aihot-eval-implementation.md)，补充后续建题、七个指标定义、文本判官及归档的接续说明。此次仅补文档和指标元数据，未运行新评测或改变生产行为。

## 2026-09-16（已启用独立原始采集）

- 经用户批准，本机 collector 已独立按每 15 分钟调度，主处理继续使用原锁。09:30 真实调度中主 pipeline 因锁跳过，collector 仍完成抓取；未改变评分、分类或精选逻辑。
- 修复主库已确认数据后、清理队列副本遇采集写锁导致导入退出的问题：仅对 SQLITE_BUSY 延后清理，继续导入；下一轮凭确认记录清理，不重复应用。生产启用与验证边界见[持续采集说明](docs/operations/continuous-eval-data.md)。

## 2026-09-16（修复重启后公网数据持续停更）

- 数据同步不再被重启或进程异常结束后留下的锁目录永久阻塞；下一轮可自动继续。保留每五小时同步和现有失败告警，并修复遗留锁使同步日志无法裁剪的问题。恢复与锁的运维边界见 [服务说明](docs/operations/services.md#db-sync-职责验证与故障证据)。

## 2026-09-16（独立采集，默认关闭）

- 新增独立 collector 与事务性待投递队列，启用后 AI 处理占锁不再阻塞下一轮采集；过滤前 raw 留档保持原格式，旧 T5 不接入。主库导入保留写锁、顺序与幂等，游标不越过尚未持久保存的投递数据。
- A4 继续取真实采集完成读数，W1 恢复要求完整采集和处理证据；启用与回退前置条件见[持续采集说明](docs/operations/continuous-eval-data.md)。本条为本地实现，生产调度尚未切换。

## 2026-09-15（已启用连续原始数据采集）

- 新增 Radar 过滤前完整正文留档、304 依赖验证、缺轮审计、冻结复制与显式 30 天清理入口；旧 T5 不作为新原始数据。代码默认关闭，本机已获批开启，不改变现有评分与判官。
- AIHOT 新增 opt-in 缺窗补收和 window_v2，保留 v1 语义及已发布窗口；网络超时/连接失败在原限流与预算内重试。用户随后批准启用：本机已打开 Radar 原始留档与 AIHOT 新模式，抓取频次不变；操作与验证边界见 [持续采集说明](docs/operations/continuous-eval-data.md)。
- 修复系统 Python 3.9 下保留期限计算失败；当前 cron 使用的 Python 3.14 语义不变。另经批准安装每日 03:07 Radar 过期原始数据清理，冻结集保留，本轮未立即删除真实数据。

## 2026-09-14（AIHOT 原始 capture 默认保留 30 天）

- AIHOT 每日采集保存的原始 capture 默认保留期从 14 天延长到 30 天；`AIHOT_CAPTURE_RETAIN_DAYS` 仍可显式覆盖，`0` 仍表示全部保留。评测 run 独立维持 14 天，validated windows 与抓取频率不变；已经清理的旧 capture 不会自动回填。

## 2026-09-14（飞书告警只在状态变化时通知，并收紧误报与假恢复）

- 同一事故不再每 30 分钟重复提醒；只在新事故、严重度变化和可证明恢复时通知，恢复统一走 notice。发送失败仍会重试首次通知，不会被下一轮暂时缺样本吞掉。
- A4 不再在午夜当天首轮抓取完成前因 `items = 0` 误 page；当天已出现明确出网 preflight 失败或任意完整抓取轮后仍会及时上膛，后续不完整轮不会抹掉真实断流证据。
- A2 仅 P95 尾延迟越线降为 notice，错误率或 pipeline 心跳越线仍为 page；A7 单个可处置静默源降为 notice，两个及以上仍为 page，且连续告警期间故障来源完全换批会开启新事故并再次通知。
- A1 样本不足、A2 三阶段全无样本、A6 计量不可评估、PERF 没有新鲜样本时不再误报恢复；无因果身份的 A1/A2/A5 共现合并已删除，消息补充用户影响和可执行的排查入口。
...
- **但真正的代价不是那条假日期，是首页约 27 小时几乎没再收进新内容。** 精选的 40 格里有 36 格留给"新鲜度"，而新鲜度只认**最新那一天**。一条未来戳独占了那一天，于是 36 个新鲜槽只用掉 1 个，剩下的从**全部历史**里按分数补——9 月 13 日凌晨连续多个 run 的入选名单里，最早的条目是 **2024-12-18**。**首页第一页并没有因此显示那些旧条目**：它是跨轮累积的归档、按发布时间取前 40，实测那段时间第一页的跨度仍是 `09-11 21:59Z` → `09-13 03:11Z`。它表现为**不再往前走**——那段窗口里新入库的 2203 条中只有 **1 条**被选进精选（恢复后同一口径是 26/2411）。这个退化**不报错、不告警**，页面看起来只是"有点旧"。
- **现在 RSS 入库不再接受晚于当前时刻的发布时间**，超出就取解析到它的那一刻。那一条已改回它实际被抓到的时间（9 月 12 日 08:45 CST，由当天的抓取日志定位），新鲜度配额随即恢复正常（随后两个 run 的最早入选条目由 2024-12-18 变为 2026-02-12）。
- **边界说清楚**：只改了 RSS 这条入库路径。`web.py` 里同形的缺口**有意没修**——全库实测 0 条 web 源越界条目，没有证据支持预防性扩大改动。另外，被钳住的原始 pubDate **不保留**，所以这一步不可确定恢复（细节见 ADR）。
- **`xai_news`（xAI 官方发布页）已暂停抓取，历史文章仍可见。** 它自 2026-09-08 起每轮都返回 403，约每天 96 次注定失败的请求。查清了：x.ai 的 Cloudflare 以 `Blocked due to abusive traffic patterns` 硬拦我们，**换 UA、带 cookie、换出口 IP（`43.153.x.x` 与 `70.231.x.x` 两个不同公网 IP）、headless 浏览器执行 JS，四者拿到的都是同一条硬 block**（正文 39 字符，challenge 根本没进场）。这四条路都不是我方能绕开的；**未测**的还有住宅 / 别的 ASN 出口、带用户 profile 的真实 Chrome、第三方镜像、以及 X 上的 @xai 账号，所以这不是"做不到"的结论，只是这四条走不通。按既有契约标为 `paused`：停掉无意义请求与静默告警，已入库的 84 条历史文章在搜索与详情里照常可见。x.ai 哪天放开，取消暂停即可恢复。
- **那 27 小时里本该上首页的内容不回填。** 首页是跨轮累积的归档，回填会改写它的历史成员资格，而那正是衡量"与参照产品靠近程度"的唯一观测面，改了就前后不可比。这个缺口已记入 ADR。
...
## 2026-09-10（精选里的论文排得更靠后：占比从约 14% 降到约 12%）

- **论文类条目排名下调 5%。** 多天合并看，精选里论文的占比从约 **14.2% 降到约 12.0%**，参照产品 AIHOT 自己是 **8.2%**。其余四类的**系数没有改动**，但它们的占比会相应上升——腾出的位置由排在后面的条目补上，单天补给谁不固定。这些数字可以自己复现：`uv run python scripts/eval/measure_curated_composition.py`，加 `--multiplier paper=1.0` 看不加系数的对照。
- **它只改排名先后，不改「够不够进精选」。** 论文照常按自己的原始分过 6.5 阈值与新鲜度地板，只是在同分段里排得更靠后。（先前的实现把系数直接乘进分数，等于把论文的准入线抬到 6.84 分（0.80 时是 8.125 分）——那是一条类别专属的收录规则、不是排序调整。实测那一版让 219 条整个掉出候选池，**而页面结果与现在完全一样**，所以它纯属多余的副作用，已在上线前改掉。）
- **但显示的分数会变**：精选页的显示分是按最终名次线性映射出来的，所以名次下移的论文显示分也会降低、徽章可能掉一档。记入档案的原始分不含这项系数（阈值、去重、审计记录读的都是它），会变的只有名次以及由名次派生的那些东西。
- **为什么**：我们的打分把论文放在候选池的 **71 分位，而 AIHOT 把同一批论文放在 47 分位**（同一批 2000 条、用 AIHOT 自己的分类标签比的）。原因在维度上看得见——论文的「信息密度」6.30 和「来源权威」6.91 都是全场最高，而这两项在排序里各占 0.40 和 0.10 的权重。**一篇又密又权威的预印本，对一个新闻摘要来说通常不是新闻。**
- **这与 9 月 9 日当天上线又撤回的那次，证据结构完全不同**，三处都换了：拟合目标从**构成占比**换成**逐条比对 AIHOT 自己的 0–100 分**（占比这个量，一个完全不看输入、按固定比例乱吐的规则也能拿满分）；**拟合用的**分类标签全部来自同一版分类 prompt，正是上次撤回时写进代码的那条硬前提（**施加时做不到这一点**：线上读的是每条最新的分类结果，其中约 28% 的论文标签出自更早的版本——已实测那批标签的准确率并不更差，但这是当下的巧合，已记进 issues）；验证从单窗口换成 **20 次半分留出，20 次全部为正**（逐条相关性 0.298→0.368）。
- **顺带修正了一件更基础的事：单天的构成数字读不出改进。** 精选每天只有几十条，AIHOT 自己每天的构成与它自己的月均值就差 0.269，而纯粹的抽样随机性已经能造出 0.253——两者分不开。所以此后这类判断一律看**多天合并**，不看单天。
- **刻意没有调到最优**：把论文压得更低，逐条相关性还会继续变好，但那是因为该指标奖励"把一个被系统性高估的类整体推到底部"。AIHOT 实际会选论文（8.2%）。合并口径上更狠的 0.80 只把总变差多压 0.006（在噪声内），**代价是在当时在跑的那个窗口上页面一篇论文都没有**——40 格里 36 格来自最新一天的前 36 名，给一类降权会让它一步跨过这个切口。取 0.95 后当前池子上是 3/40，参照值 8.2% 对应 3.3/40。
- **顺带纠正一个此前的误判**：精选里模型发布的占比看着比参照产品低得多（12% 对 30%），此前判成排序缺陷。**不是。** 同一批候选、同一套打分，只改「每天取几条」：取到参照产品那个深度（每天十几条）时我们是 **29.2%**，与它的 30.2% 基本相同；取到我们自己的 40 条时降到 19.8%，取满整池降到 13.7%。**模型类的条目分数高、集中在最顶端，版面越长越被稀释**——这是两个产品版面长度不同（我们 40 条 / 它十几条），不是排序排错了。同深度下真正还超配的是行业新闻（+8.5pp）与论文（+5.7pp），后者正是本次改的这一条。

## 2026-09-09（精选排序的类别微调：当天上线、当天撤回）

- **上线约两小时后撤回，精选恢复原样。** 上线依据是一次配对读数（与 AIHOT 构成的总变差 0.193→0.151）；但那个读数是按**分类**算的，而各天的分类出自**当天在跑的那版分类 prompt**。把六天的分类用同一版 prompt 重算之后再比，结论翻了：六天里**四天变差**（最差 +0.132），均值 +0.020。
- **最能说明问题的是同一天**：9 月 9 日在重算前读作 −0.041（改善），重算后读作 +0.026（变差）。**当初支持上线的那个数，是在陈旧分类上量出来的。**
- **机制留着、参数清空**：候选现在带类别、排序理由里会记下用了什么系数，但系数表为空，所以排序与上线前一致。下次要再调，**必须先用同一版 prompt 重算各窗口的分类**，否则拟合的是标注口径而不是排序。

## 2026-09-09（进站判准：从「跟 AI 有关」改成「这条在讲的那件事是不是 AI」）

- **站上会少一类东西：内置了 AI 功能的消费硬件。** 手机、汽车、眼镜、家电的发布、定价、配色、上市与评测，此前会被当成 AI 内容收进来——因为旧判准问的是「跟 AI 有没有关系」，而这类东西确实有关系（车机、语音助手、AI 芯片都是 AI 系统）。实测一份 400 条的池子随机样本里，手判 25 条有 5–7 条属于这类，其中一条是某 SUV 的**新车色**首发。现在判准问的是**这条内容在讲的那一件事**是不是 AI：一部手机跟 AI 有关，但那篇文章讲的是一部手机。
- **正文为空、只有一个裸链接的条目也不再收进来**——此前判准只有是/否两档，没有「判不了」，它们必然被塞进某一侧。
- **判准改了会自动生效，不用再记得改版本号。** 此前进站判准的版本戳是一个手工维护的日期，停在 5 月 13 日；而「这条要不要重判」正是按那个戳比对的，所以判准改了、存量永远不会重判。现在戳从判准本身算出来，改了就变。（enrich 的分类判准 9 月 8 日刚修过同一个缺陷。）
- **随之而来的重算是有界的**：戳一动，24 小时窗口内约 7000 条要重判。每轮上限 400 条（约 10 分钟），并且**从没判过的新文章永远排在重判前面**——否则新文章会排在几千条重判之后、在轮到之前就滑出窗口。整批约 21 轮、5–6 小时排空。

## 2026-09-09（出网从「证明选择器的状态」换成「证明这条出口真的通」）

- **一次 9 小时的零摄入，是出网检查自己造成的。** 09-08 21:41 本机出网模式被改回 clash，而 pipeline 的出网检查断言的是 domain-router 的**存储模式**，于是它 fail closed 了 35 轮、零摄入 8.8 小时——而代理全程好好的：21:30 那轮在模式被改之后仍把抓取跑到 23:28、把分类跑到 23:37，全部成功。这是一次假阴性，而那套检查无法把它和真故障区分开。
- **现在改成：自有流量一律从一个本地端口出去，检查经它实发一次请求。** 端口是 `AI_RADAR_EGRESS_PROXY_PORT`（默认 59527），验通目标是 `AI_RADAR_EGRESS_PROBE_URL`（默认 `https://api.github.com/zen`，特意选与本站抓取对象无关的端点）。**不用端口探活**——2026-08-18 实测过一次：日志头部代理行看着完全正常，同轮 162 个源全部 `Connection refused`；端口探测区分不了「本地 listener 活着」与「上游隧道通」。
- **失败时的话变了，能照着做**：以前说「恢复一个健康的 domain-routing selector」，现在点名端口、给出 `lsof -nP -iTCP:<端口> -sTCP:LISTEN`、并说明改端口用哪个环境变量。A2 告警的处置文案同步更新。
- **放弃了一项性质，说清楚**：以前应用对「哪个域名走哪条线路」有一份 fail-closed 的契约（Anthropic→GCP SG、OpenAI→Tencent primary…），现在路由完全由监听那个端口的东西决定，应用既不约束也观测不到。
- **升级注意**：`policy_sha256` 改为按出口端口派生，因此 `$AI_ASSISTANT_ROOT` 里那份 selector 兼容性收据**必然失效**。端口没人在听时表现为 `interpret FAIL`（响）；**端口一旦立起来，就变成静默跳过、pipeline 照打 `interpret OK`**，直到 A5 在 4 小时后开火。重新签发的命令与各端口对应的新 sha 见 `docs/experiences/integration.md`。
