...
## 2026-09-15（可选连续原始数据采集，尚未启用）

- 新增 Radar 过滤前完整正文留档、304 依赖验证、缺轮审计、冻结复制与显式 30 天清理入口；旧 T5 不作为新原始数据。默认关闭，不改变现有评分与判官。
- AIHOT 新增 opt-in 缺窗补收和 window_v2，保留 v1 语义及已发布窗口；网络超时/连接失败在原限流与预算内重试。新常驻配置需要另行批准，操作与验证边界见 [持续采集说明](docs/operations/continuous-eval-data.md)。
- 修复系统 Python 下保留期限计算失败；部署后既有 30 天过期清理将恢复执行，本轮未删除真实数据。
