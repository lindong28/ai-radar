...
- 标签数量不合规不再丢掉整篇。模型被要求写 2-4 个标签，写多了或写少了此前都会让整条 enrich 输出被拒——而代码在下一步本来就只保留前四个，写少了也还有来源推导出的标签可以补。全量实测 2741 篇里有 75 篇栽在这一条上，是最大的一类失败（多与少各占多少无从得知：被拒时模型的原始标签没有落盘）。现在多的截掉、少的交给下面那道「有效标签不足两个才拒收」的底线，摘要、推荐理由、分类照常保留。截断在按词表过滤和去重之后才做，否则排在第五位的合规标签会先被位置截掉、只留下一个随后也要被丢弃的越界标签。
