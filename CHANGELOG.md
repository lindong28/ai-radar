...
- **`xai_news`（xAI 官方发布页）已暂停抓取，历史文章仍可见。** 它自 2026-09-08 起每轮都返回 403，约每天 96 次注定失败的请求。查清了：x.ai 的 Cloudflare 以 `Blocked due to abusive traffic patterns` 硬拦我们，**换 UA、带 cookie、换出口 IP（两个完全不同的公网 IP）、真浏览器执行 JS，四种都一样**——不是我方可修的问题。按既有契约标为 `paused`：停掉无意义请求与静默告警，已入库的 84 条历史文章在搜索与详情里照常可见。x.ai 哪天放开，取消暂停即可恢复。
