...
## 2026-09-17（采集监督清理竞态修复，启用待许可）

- 修复子脚本退出后残留进程组清理的 macOS EPERM 竞态：有界等待全组退出，不把真实权限失败当成成功。
- 监督内部异常保存失败终态，纳入已有独立健康告警；重试开始不再清除尚未恢复的故障，超预算未收尾也会报错。不改变原始输入范围、来源或调度频次；现场启用状态见[持续采集说明](docs/operations/continuous-eval-data.md)。
