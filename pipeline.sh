...
# /usr/sbin is not on cron's PATH. This was added because the domain-selector
# status helper shelled out to bare `lsof` and died without it, taking
# egress-preflight down with it (2026-09-05, 4+ rounds; symptom was a healthy
# manual run and a failing cron run). That path is gone -- egress-preflight now
# probes a port directly and runs no helper -- but the export stays: other
# stages and any operator command run from here still expect a login-like PATH,
# and narrowing it is a change with a blast radius nobody has measured.
